...
function renderSpRecentScore(score) {
  if (score === null || score === undefined) return `<span class="sp-recent-pill sp-recent-neutral">—</span>`;
  return `<span class="sp-recent-pill ${spRecentClass(score)}">${fmtDecimal(score, 1)}</span>`;
}
      ...
      <div class="table-wrap">
        <table class="performance-table">
          ...
          <tbody>
            ...
            ${renderComponentRow("Pitcher Vulnerability", mlbComponentAnalysis?.avg_pitcher_vuln_when_hit, mlbComponentAnalysis?.avg_pitcher_vuln_when_no_hit)}
        ...
        <table class="performance-table">
          ...
          <tbody>
            ${bestPitcherHitters.length ? bestPitcherHitters.map((row, index) => {
              ...
              `;
            }).join("") : `<tr><td colspan="8" class="empty-state">Loading target pitcher hitters...</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
