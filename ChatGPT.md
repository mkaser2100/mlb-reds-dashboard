...
## V2 Opportunity-Adjusted Model

### Purpose

V2 is an experimental rules-based model designed to improve V1 while remaining simple, explainable, and fast. Following a feature overlap audit, V2 was simplified to avoid double-counting signals that were already embedded within the V1 Matchup Score.

### Current Logic

```
V2 Score = (80% × V1 Matchup Score) + (20% × Expected Plate Appearance Score)
```

### Why this changed

A feature importance and overlap audit showed that the V1 Matchup Score already incorporates much of the predictive value from:
...
Earlier versions of V2 added these signals a second time, creating unnecessary redundancy. The audit found that **Expected Plate Appearance (derived from projected lineup position)** was the strongest independent feature not already represented by V1.

### Current Characteristics

- Uses V1 Matchup Score as the primary ranking engine.
- Adds only Expected Plate Appearance as an independent adjustment.
- Eliminates intentional double-counting of recent form and split-based metrics.
- Remains a transparent heuristic benchmark alongside V1 and V3.
