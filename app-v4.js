...
const V2_ENHANCEMENT_SELECT = "player_id,game_pk,model_v2_score,calibrated_hit_probability,probability_lift_vs_base,model_confidence,expected_plate_appearances,recent_lineup_spot,adjusted_batter_split_avg,adjusted_pitcher_baa_split,adjusted_batter_split_score,adjusted_pitcher_vulnerability_score";
let searchTerm = "";
...
function mergeV2Enhancements(rows, v2Rows) {
  ...
  (v2Rows || []).forEach((row) => {
    ...
    byPlayer.set(String(row.player_id), row);
    if (row.game_pk) byPlayerGame.set(v2LookupKey(row), row);
  });
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      calibrated_hit_probability: v2.calibrated_hit_probability,
      probability_lift_vs_base: v2.probability_lift_vs_base,
...
function renderTable() {
  ...
  if (!rows.length) {
    setHtml("hittersTableBody", `
      <tr>
        <td colspan="13" class="empty-state">No hitters match your search.</td>
      </tr>
    `);
    return;
  }
  ...
  const html = rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-player-id="${row.player_id}">
        ...
        <td class="num">${matchup?.model_v2_score !== undefined ? `<span class="model-v2-badge">${fmtDecimal(matchup.model_v2_score, 1)}</span>` : "—"}</td>
        <td class="num">${matchup?.calibrated_hit_probability !== undefined ? `<span class="probability-badge">${fmtProbability(matchup.calibrated_hit_probability)}</span>` : "—"}</td>
...
function renderScoreBreakdown(matchup) {
  ...
  const v2Summary = matchup.model_v2_score !== undefined || matchup.calibrated_hit_probability !== undefined
    ? `
      <div class="v2-summary">
        <div>
          <span>V2 Pick Score</span>
          <strong>${matchup.model_v2_score !== undefined ? fmtDecimal(matchup.model_v2_score, 1) : "—"}</strong>
        </div>
        <div>
          <span>Estimated Hit Probability</span>
          <strong>${matchup.calibrated_hit_probability !== undefined ? fmtProbability(matchup.calibrated_hit_probability) : "—"}</strong>
        </div>
        <div>
...
function openDrawer(playerId) {
  ...
  setText("drawerV2PickScore", matchup?.model_v2_score !== undefined ? fmtDecimal(matchup.model_v2_score, 1) : "—");
  setText("drawerHitProbability", matchup?.calibrated_hit_probability !== undefined ? fmtProbability(matchup.calibrated_hit_probability) : "—");
  setText(
    "drawerPitcher",
...
function openMlbDrawer(playerId) {
  ...
  setText(
    "drawerPlayerSub",
    `${row.team_name || "MLB"} · Player ID ${playerId} · Last ${mlbWindow} games + today's matchup split`
  );

  setText("drawerMatchupScore", fmtDecimal(row.matchup_score, 1));
  setText("drawerV2PickScore", row.model_v2_score !== undefined ? fmtDecimal(row.model_v2_score, 1) : "—");
  setText("drawerHitProbability", row.calibrated_hit_probability !== undefined ? fmtProbability(row.calibrated_hit_probability) : "—");
  setText(
    "drawerPitcher",
  ...
  // MLB drawer uses the recent production fields returned by get_today_mlb_batter_matchups.
  ...
  // MLB drawer only has the current matchup split from the all-MLB RPC.
  // Put that split into the relevant handedness tile and blank the others intentionally.
...
async function loadMlbHitBoardData() {
  try {
    ...
    mlbRows = mergeV2Enhancements(baseRows, v2Rows);
  ...
  content.innerHTML = `
    <section class="control-deck performance-window-deck">
      ...
      <div class="control-group grow">
        ...
        <div class="sort-pill">All MLB hitters · V1 score plus V2 probability overlay</div>
      </div>
        ...
        <table class="performance-table">
          <thead>
            <tr>
              ...
              <th class="num">V2 Pick</th>
              <th class="num">Hit Prob</th>
              <th class="num">Hot</th>
            ...
            ${bestPitcherHitters.length ? bestPitcherHitters.map((row, index) => {
              ...
              return `
                <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}">
                  ...
                  <td class="num">${row.model_v2_score !== undefined ? `<span class="model-v2-badge">${fmtDecimal(row.model_v2_score, 1)}</span>` : "—"}</td>
                  <td class="num">${row.calibrated_hit_probability !== undefined ? `<span class="probability-badge">${fmtProbability(row.calibrated_hit_probability)}</span>` : "—"}</td>
              ...
              `;
            }).join("") : `<tr><td colspan="10" class="empty-state">Loading target pitcher hitters...</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
        ...
        <table>
          ...
          <tbody>
            ${top20.length ? top20.map((row, index) => {
              ...
              `;
            }).join("") : `<tr><td colspan="13" class="empty-state">Loading MLB matchup scores...</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
