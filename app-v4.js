...
console.info("MLB Hit Lab app-v4 loaded: v3-driver-live-hydration-v3");
...
function resolveV3DriverPresentation(item, direction) {
  ...
  const description = metadata?.describe
...
async function hydrateV3DrawerExplanation(row) {
  const hasDrivers = Array.isArray(row?.drawer_top_positive_drivers) && row.drawer_top_positive_drivers.length;
  const hasRisks = Array.isArray(row?.drawer_top_risk_factors) && row.drawer_top_risk_factors.length;
  if (hasDrivers || hasRisks) return row;

  const predictionDate = row?.prediction_run_date;
  const playerId = row?.player_id;
  if (!predictionDate || !playerId) return row;
  ...
  try {
    ...
    return mergeV3DrawerExplanations([row], [data])[0] || row;
  } catch (err) {
    console.error("Error hydrating V3 drawer explanation on demand:", err);
    return row;
  }
}

async function openV3MlbDrawer(row) {
  row = await hydrateV3DrawerExplanation(row);
  ...
  body.innerHTML = `
    ...
    <section class="drawer-section v3-section v3-driver-section">
      ...
      ` : (!hasExactContributions && v3RiskPills(row)) ? `
        <div class="v3-driver-divider"></div>
        ${v3RiskPills(row)}
      ` : ""}
...
function normalizeV3HitRow(row) {
  ...
  return {
    ...
    drawer_top_positive_drivers: Array.isArray(row.drawer_top_positive_drivers) ? row.drawer_top_positive_drivers : [],
    drawer_top_risk_factors: Array.isArray(row.drawer_top_risk_factors) ? row.drawer_top_risk_factors : [],
