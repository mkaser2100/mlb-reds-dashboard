...
console.info("MLB Hit Lab app-v4 loaded: v3-driver-date-lock-v3");
...
function mergeV3DrawerExplanations(rows, explanationRows = []) {
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      drawer_group_contributions: Array.isArray(explanation.group_contributions) ? explanation.group_contributions : [],
      drawer_all_features: Array.isArray(explanation.all_features) ? explanation.all_features : [],
...
function openV3MlbDrawer(row) {
  ...
  const groupContributions = row.drawer_group_contributions || [];
  const hasExactContributions = positiveDrivers.length || riskFactors.length || groupContributions.length;
