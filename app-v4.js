...
console.info("MLB Hit Lab app-v4 loaded: team-hit-board-phase2-v1");
...
// Temporary aliases retained so unrelated legacy functions do not break during
// the Team Hit Board transition. New page code uses the generic names above.
...
function selectedGamePitcherRow() {
  const rows = selectedGameBoardRows();
  const preferred =
    rows[0] ||
    teamGameRows.find((row) => row.hitter_side === selectedTeamSide) ||
    teamGameRows[0];
...
async function loadSelectedGameHotStats() {
  try {
    ...
    // Compatibility aliases during Phase 2.
    redsGameHotRows = teamGameHotRows;
  } catch (err) {
...
async function loadTeamGameSelector() {
  ...
  if (!selectedStillExists) {
    const redsGame = availableTeamGames.find(
      (game) =>
        Number(game.home_team_id) === TEAM_ID ||
        Number(game.away_team_id) === TEAM_ID
    );
...
async function loadTeamGameBoardData() {
  try {
    ...
    await loadSelectedGameHotStats();
  } catch (err) {
...
// Compatibility wrapper for existing page-load calls.
async function loadRedsGameBoardData() {
  return loadTeamGameBoardData();
}
...
function handleTeamSideChange(side) {
  ...
  redsTeamFilter = selectedTeamSide;
  renderTeamBoardPage();
}
  ...
  return `
    <section class="team-game-selector-card" aria-label="Team Hit Board game selector">
      <div class="team-game-selector-copy">
        ...
        <span>The entire page updates to the selected game.</span>
      </div>
...
function renderTeamBoardControls() {
  ...
  return `
    <section class="control-deck performance-window-deck team-v3-controls">
      <div class="control-group">
      ...
      <div class="control-group grow">
        ...
        <div class="sort-pill">${escapeHtml(teamGameLabel(game))} · ${escapeHtml(teamGameSideLabel(selectedTeamSide))}</div>
      </div>
...
// Compatibility wrapper retained until Phase 4 cleanup.
function renderRedsBoardControls() {
  return renderTeamBoardControls();
}
...
function renderRedsMlRows(rows) {
  return renderTeamMlRows(rows);
}

function renderMatchupHero() {
  const game = selectedTeamGame();
  const pitcherRow = selectedGamePitcherRow();
  ...
  setText("matchupPitcher", `${formatGameDate(game.game_date)} · ${formatTeamGameTime(game)}`);
  setText("pitcherRoleLabel", "Pitcher Facing Top Signal");
  setText("pitcherEraLabel", "ERA");
  setText("pitcherWhipLabel", "WHIP");

  setText("pitcherNameCard", pitcherRow?.pitcher_name || "Probable Starter TBD");
  setText("pitcherThrows", pitcherRow?.pitcher_throws || "—");
  setText(
    "pitcherEra",
    fmtDecimal(
      pitcherRow?.pitcher_season_era ??
      pitcherRow?.pitcher_last5_era,
      2
    )
  );
  setText(
    "pitcherWhip",
    fmtDecimal(
      pitcherRow?.pitcher_season_whip ??
      pitcherRow?.pitcher_last5_whip,
      2
    )
  );

  const handSplits = mlbTargetPitcherSplits?.[String(pitcherRow?.pitcher_id)] || {};
  setText(
    "pitcherVsLhb",
    fmtAvg(
...
function renderTeamBoardPage() {
  ...
  setHtml("redsBoardControls", renderTeamBoardControls());
  setText("redsBoardTitle", `${teamGameLabel(game)} Hit Probabilities`);
  setText("redsBoardEyebrow", "Selected Game · V3 Team Board");
