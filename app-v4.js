...
console.info("MLB Hit Lab app-v4 loaded: performance-normalized-data-v1");
  ...
  try {
    const [
      ...
      safePerformanceQuery(
        "v_mlb_app_model_scorecard",
        client
          .from("v_mlb_app_model_scorecard")
          .select("*")
      ...
      safePerformanceQuery(
        "v_mlb_app_rolling_model_performance",
        client
          .from("v_mlb_app_rolling_model_performance")
          .select("*")
      ...
      safePerformanceQuery(
        "v_mlb_app_model_score_buckets",
        client
          .from("v_mlb_app_model_score_buckets")
          .select("*")
          .order("model_key", { ascending: true })
    ...
    performanceDailyScorecardRows = scorecardData || [];
    performanceRollingBucketRows = rollingData || [];
    performanceCalibrationRows = bucketData || [];
    performanceV3CalibrationRows = [];
    performanceComponentRows = featureSignalData || [];
    ...
    console.info("Model Performance rows loaded", {
      ...
      buckets: performanceCalibrationRows.length,
      featureSignals: performanceComponentRows.length
    });
...
function aggregateScorecardRows(rangeKey) {
  return (performanceDailyScorecardRows || [])
    .filter((row) => row.range_key === rangeKey)
    .map((row) => ({
      model_key: normalizePerformanceModelKey(row.model_key),
      model_name: row.model_name,
      days: Number(row.days || 0),
      top1: pctValueFromRatioOrPct(row.top1_hit_rate_pct),
      top5: pctValueFromRatioOrPct(row.top5_hit_rate_pct),
      top10: pctValueFromRatioOrPct(row.top10_hit_rate_pct),
      top20: pctValueFromRatioOrPct(row.top20_hit_rate_pct),
      top25: pctValueFromRatioOrPct(row.top25_hit_rate_pct),
      overall: pctValueFromRatioOrPct(row.overall_hit_rate_pct)
    }))
    .filter((row) => PERFORMANCE_MODEL_DEFS[row.model_key]);
}
...
function calibrationRowsForModel(modelKey) {
  return (performanceCalibrationRows || [])
    .filter((row) => normalizePerformanceModelKey(row.model_key) === modelKey)
    .map((row) => ({
      model_key: modelKey,
      bucket_label: row.bucket_label || "Unknown",
      bucket_order: Number(row.bucket_order || 999),
      predictions: Number(row.players || 0),
      hits: Number(row.hits || 0),
      hit_rate: pctValueFromRatioOrPct(row.hit_rate_pct)
    }))
...
function featureRowsForModel(modelKey) {
  return (performanceComponentRows || [])
    .filter((row) => normalizePerformanceModelKey(row.model_key) === modelKey)
    .map((row) => ({
      feature: row.feature_name || "Unknown",
      value: Number(row.signal_value || 0),
      detail: `${Number(row.signal_value || 0) > 0 ? "+" : ""}${fmtDecimal(row.signal_value, Math.abs(Number(row.signal_value || 0)) < 1 ? 2 : 1)}`
    }))
  ...
  return `
    <section class="board-card performance-card feature-importance-card">
      <div class="board-header">
        ...
        <div class="board-meta">
          <span>${performanceModelFilter === "v3_ml" ? `${fmtNum(featureRowsForModel("v3_ml").length)} V3 signals` : `Last ${performanceWindow} model window`}</span>
