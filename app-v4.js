...
console.info("MLB Hit Lab app-v4 loaded: market-edge-action-pill-v4");
...
function renderMarketEdgeTierBadge(row) {
  ...
  return `<span class="market-action-pill ${tier.className}" title="${escapeHtml(tier.label)}"><span class="market-action-dot">${tier.emoji}</span><span>${escapeHtml(tier.label)}</span></span>`;
}
