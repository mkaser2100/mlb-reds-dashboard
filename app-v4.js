...
function renderMlbHitBoardPerformanceSection() {
  ...
  return `
    <section class="performance-note">
      ...
      <span>Uses the same rolling prediction history for the table and KPI cards. Historical backtest rows plus new daily rows are included; no-AB players are excluded.</span>
    </section>
    ...
    <section class="hero-grid performance-grid">
      <article class="insight-card compact">
        ...
        <div class="metric-value">${fmtRate(mlbStats?.top_pick_hit_rate)}</div>
        <div class="metric-sub">${fmtNum(mlbStats?.top_pick_abs)} AB opportunities</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">🥇</div>
        <div class="metric-label">MLB Top 5</div>
        <div class="metric-value">${fmtRate(mlbStats?.top_5_hit_rate)}</div>
        <div class="metric-sub">${fmtNum(mlbStats?.top_5_abs)} AB opportunities</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">📊</div>
        <div class="metric-label">MLB Top 20</div>
        <div class="metric-value">${fmtRate(mlbStats?.top_20_hit_rate)}</div>
        <div class="metric-sub">${fmtNum(mlbStats?.top_20_abs)} AB opportunities</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">🎯</div>
        <div class="metric-label">Target SP Top 5</div>
        <div class="metric-value">${fmtRate(mlbStats?.target_sp_top_5_hit_rate)}</div>
        <div class="metric-sub">${fmtNum(mlbStats?.target_sp_top_5_abs)} AB opportunities</div>
      </article>
    </section>
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        ...
        <div class="board-meta">
          <span>${fmtNum(mlbStats?.scored_player_games)} predictions stored</span>
          <span>${fmtNum(mlbStats?.ab_opportunities)} AB opportunities</span>
        </div>
      </div>
      ...
      <div class="table-wrap">
        <table class="performance-table">
          ...
          <tbody>
            <tr><td>Top Pick</td><td class="num">${fmtNum(mlbStats?.top_pick_abs)}</td><td class="num">${fmtRate(mlbStats?.top_pick_hit_rate)}</td></tr>
            <tr><td>Top 5</td><td class="num">${fmtNum(mlbStats?.top_5_abs)}</td><td class="num">${fmtRate(mlbStats?.top_5_hit_rate)}</td></tr>
            <tr><td>Top 10</td><td class="num">${fmtNum(mlbStats?.top_10_abs)}</td><td class="num">${fmtRate(mlbStats?.top_10_hit_rate)}</td></tr>
            <tr><td>Top 20</td><td class="num">${fmtNum(mlbStats?.top_20_abs)}</td><td class="num">${fmtRate(mlbStats?.top_20_hit_rate)}</td></tr>
            <tr><td>Target SP Top 5</td><td class="num">${fmtNum(mlbStats?.target_sp_top_5_abs)}</td><td class="num">${fmtRate(mlbStats?.target_sp_top_5_hit_rate)}</td></tr>
            <tr><td>All Stored MLB Predictions</td><td class="num">${fmtNum(mlbStats?.ab_opportunities)}</td><td class="num">${fmtRate(mlbStats?.overall_hit_rate || mlbStats?.all_hit_rate)}</td></tr>
          </tbody>
        </table>
      </div>
    </section>
