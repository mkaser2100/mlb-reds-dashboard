...
console.info("MLB Hit Lab app-v4 loaded: team-hit-board-phase4a-generic-names");
...
Object.defineProperties(window, {
  redsTeamFilter: {
    configurable: true,
    get: () => selectedTeamSide,
    set: (value) => {
      selectedTeamSide = ["all", "home", "away"].includes(value) ? value : "all";
    }
  },
  redsMlRows: {
    configurable: true,
    get: () => teamGameRows,
    set: (value) => {
      teamGameRows = Array.isArray(value) ? value : [];
    }
  },
  redsGameHotRows: {
    configurable: true,
    get: () => teamGameHotRows,
    set: (value) => {
      teamGameHotRows = Array.isArray(value) ? value : [];
    }
  }
});

// Phase 4A keeps generic state authoritative. Compatibility accessors are
// exposed on window for older integrations without maintaining duplicate state.
...
const TEAM_BOARD_LEGACY_IDS = {
  teamBoardControls: "redsBoardControls",
  teamBoardEyebrow: "redsBoardEyebrow",
  teamBoardTitle: "redsBoardTitle",
  teamBoardTableWrap: "redsBoardTableWrap"
};

function teamBoardDomId(genericId) {
  if ($(genericId)) return genericId;
  return TEAM_BOARD_LEGACY_IDS[genericId] || genericId;
}

function setTeamBoardText(genericId, value) {
  setText(teamBoardDomId(genericId), value);
}

function setTeamBoardHtml(genericId, value) {
  setHtml(teamBoardDomId(genericId), value);
}
...
function selectedGamePitcherRow(side = selectedPitcherSide) {
  ...
  return (
    ...
    teamGameRows.find((row) => row.pitcher_id) ||
    null
  );
}
...
// Deprecated compatibility wrapper for older callers; remove in Phase 4B.
...
function renderMatchupHero() {
  ...
  const pitcherRow = pitchers[selectedPitcherSide] || pitchers.home || pitchers.away;
...
// Deprecated compatibility wrapper for cached markup; remove in Phase 4B.
function openRedsV3Drawer(playerId) {
  openTeamV3Drawer(playerId);
}
...
function wireEvents() {
  ...
  document.addEventListener("click", async (event) => {
    const legacyTeamButton = event.target.closest("[data-reds-team-filter]");
    if (legacyTeamButton) {
      event.preventDefault();
      event.stopPropagation();
      const nextFilter = legacyTeamButton.dataset.redsTeamFilter;
      if (nextFilter && nextFilter !== selectedTeamSide) {
        handleTeamSideChange(nextFilter);
      }
      return;
    }
    ...
    // Backward compatibility for cached rows from the former Reds-specific page.
    if (row.dataset.redsV3PlayerId) {
      openTeamV3Drawer(row.dataset.redsV3PlayerId);
      return;
    }
