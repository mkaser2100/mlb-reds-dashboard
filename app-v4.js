...
async function loadHotData() {
  try {
    ...
    setHtml("hittersTableBody", `
      <tr>
        <td colspan="11" class="empty-state">Loading Reds hitter data...</td>
      </tr>
    `);
...
function renderTable() {
  ...
  if (!rows.length) {
    setHtml("hittersTableBody", `
      <tr>
        <td colspan="11" class="empty-state">No hitters match your search.</td>
      </tr>
    `);
    return;
  }
...
function renderScoreBreakdown(matchup) {
  ...
  el.innerHTML = `
    ...
    ${breakdownRow(
      ...
      0.40,
      `Last ${matchup?.model_window || selectedWindow} games`
    )}
...
function mlbDrawerHotRow(row) {
  return {
    ...
    hit_rate: row.batter_recent_hit_rate ?? row.hit_rate,
    current_hit_streak: row.current_hit_streak,
    at_bats: row.at_bats,
    hits: row.hits,
    ...
    games_with_hit: row.games_with_hit,
    games: row.games
  };
}
...
function mlbDrawerMatchupRow(row) {
  return {
    ...
    batter_split_avg: row.batter_split_avg,
    batter_split_ab: row.batter_split_ab,
    pitcher_baa_split: row.pitcher_baa_split,
    explanation:
      row.explanation ||
...
function mlbDrawerSplitRow(row) {
  const vsLhpAvg = String(row.batter_split_label || "").includes("vs LHP") ? row.batter_split_avg : null;
  const vsRhpAvg = String(row.batter_split_label || "").includes("vs RHP") ? row.batter_split_avg : null;
  const vsLhpAb = String(row.batter_split_label || "").includes("vs LHP") ? row.batter_split_ab : null;
  const vsRhpAb = String(row.batter_split_label || "").includes("vs RHP") ? row.batter_split_ab : null;
  ...
  return {
    ...
    day_avg: row.day_avg,
    day_ab: row.day_ab,
    night_avg: row.night_avg,
    night_ab: row.night_ab
  };
}

function openMlbDrawer(playerId) {
  const row = mlbRows.find((x) => String(x.player_id) === String(playerId));
  if (!row) return;

  const hot = mlbDrawerHotRow(row);
  const split = mlbDrawerSplitRow(row);
  ...
  setText(
    "drawerPlayerSub",
    `${row.team_name || "MLB"} · Player ID ${playerId} · ${mlbWindow}-game form, matchup, and splits`
  );
  ...
  setText(
    "drawerMatchupExplanation",
    matchup.explanation || "No matchup explanation available yet."
  );
  renderScoreBreakdown(matchup);

  setText("drawerHotScore", fmtDecimal(row.hot_score, 1));
  setText("drawerAvg", row.batting_average !== undefined ? fmtAvg(row.batting_average) : "—");
  ...
  setText("drawerAb", row.at_bats !== undefined ? fmtNum(row.at_bats) : "—");
  setText("drawerHits", row.hits !== undefined ? fmtNum(row.hits) : "—");
  setText("drawerHr", row.home_runs !== undefined ? fmtNum(row.home_runs) : "—");
  setText("drawerRbi", row.rbi !== undefined ? fmtNum(row.rbi) : "—");
  setText("drawerBb", row.walks !== undefined ? fmtNum(row.walks) : "—");
  setText("drawerSb", row.stolen_bases !== undefined ? fmtNum(row.stolen_bases) : "—");

  setText("drawerLhpAvg", split.vs_lhp_avg !== null && split.vs_lhp_avg !== undefined ? fmtAvg(split.vs_lhp_avg) : "—");
  setText("drawerLhpAb", split.vs_lhp_ab !== null && split.vs_lhp_ab !== undefined ? `${fmtNum(split.vs_lhp_ab)} AB` : "— AB");

  setText("drawerRhpAvg", split.vs_rhp_avg !== null && split.vs_rhp_avg !== undefined ? fmtAvg(split.vs_rhp_avg) : "—");
  setText("drawerRhpAb", split.vs_rhp_ab !== null && split.vs_rhp_ab !== undefined ? `${fmtNum(split.vs_rhp_ab)} AB` : "— AB");

  setText("drawerHomeAvg", split.home_avg !== undefined ? fmtAvg(split.home_avg) : "—");
  setText("drawerHomeAb", split.home_ab !== undefined ? `${fmtNum(split.home_ab)} AB` : "— AB");

  setText("drawerAwayAvg", split.away_avg !== undefined ? fmtAvg(split.away_avg) : "—");
  setText("drawerAwayAb", split.away_ab !== undefined ? `${fmtNum(split.away_ab)} AB` : "— AB");

  setText("drawerDayAvg", split.day_avg !== undefined ? fmtAvg(split.day_avg) : "—");
  setText("drawerDayAb", split.day_ab !== undefined ? `${fmtNum(split.day_ab)} AB` : "— AB");

  setText("drawerNightAvg", split.night_avg !== undefined ? fmtAvg(split.night_avg) : "—");
  setText("drawerNightAb", split.night_ab !== undefined ? `${fmtNum(split.night_ab)} AB` : "— AB");
  ...
  if (drawerBody) {
    const wrapper = document.createElement("div");
    wrapper.id = "drawerBvpSection";
    wrapper.innerHTML = renderBvpSection(null, matchup);
    drawerBody.appendChild(wrapper);
  }
                ...
                <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}">
                  ...
                  <td>
                    <div class="player-cell">
                      ...
                      <div>
                        <div class="player-name">${row.full_name}</div>
                        <div class="player-sub">${row.games_with_hit}/${row.games} games with hit · ${row.batter_split_label || "split n/a"} · click for matchup</div>
                      </div>
                    </div>
                  </td>
                  <td>${row.team_name || "—"}</td>
