...
console.info("MLB Hit Lab app-v4 loaded: v3-driver-direct-fetch-v4");
...
function mergeV3DrawerExplanations(rows, explanationRows = []) {
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      drawer_positive_tags: v3JsonArray(explanation.positive_tags),
      drawer_negative_tags: v3JsonArray(explanation.negative_tags),
      ...
      drawer_top_positive_drivers: v3JsonArray(explanation.top_positive_drivers),
      drawer_top_risk_factors: v3JsonArray(explanation.top_risk_factors),
      drawer_group_contributions: v3JsonArray(explanation.group_contributions),
      drawer_all_features: v3JsonArray(explanation.all_features),
      drawer_feature_count: Number(explanation.feature_count || 0),
      drawer_contribution_data_status: "loaded"
    };
  });
}
...
async function loadV3DrawerExplanationForRow(row) {
  ...
  try {
    ...
    const exactGame = candidates.find(
      (item) => String(item?.game_pk ?? "") === String(row?.game_pk ?? "")
    );

    return exactGame || candidates[0] || null;
  } catch (err) {
...
async function openV3MlbDrawer(row) {
  ...
  if (!hasExactContributions && !row.__drawerDirectLookupAttempted) {
    ...
    if (directExplanation) {
      const mergedRow = mergeV3DrawerExplanations(
        [{ ...row, __drawerDirectLookupAttempted: true }],
        [directExplanation]
      )[0];

      return openV3MlbDrawer(mergedRow);
    }
