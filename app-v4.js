...
console.info("MLB Hit Lab app-v4 loaded: market-edge-top25-v3");
...
function renderMarketEdgeTierBadge(row) {
  ...
  return `<span class="edge-badge action-badge ${tier.className}" title="${escapeHtml(tier.label)}">${tier.emoji} ${escapeHtml(tier.label)}</span>`;
}
...
function ensureMarketEdgeTierStyles() {
  ...
  style.textContent = `
    .market-edge-table .action-badge {
      display: inline-flex;
      align-items: center;
