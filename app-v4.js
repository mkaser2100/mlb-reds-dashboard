  ...
  const html = rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-player-id="${row.player_id}">
        ...
        <td class="num">${matchup?.calibrated_hit_probability != null ? `<span class="probability-badge">${fmtProbability(matchup.calibrated_hit_probability)}</span>` : "—"}</td>
    ...
    ? `
      <div class="v2-summary">
        ...
        <div>
          ...
          <strong>${matchup.calibrated_hit_probability != null ? fmtProbability(matchup.calibrated_hit_probability) : "—"}</strong>
        </div>
        <div>
...
function openDrawer(playerId) {
  ...
  setText("drawerHitProbability", matchup?.calibrated_hit_probability != null ? fmtProbability(matchup.calibrated_hit_probability) : "—");
  setText(
    "drawerPitcher",
...
function openMlbDrawer(playerId) {
  ...
  setText("drawerHitProbability", row.calibrated_hit_probability != null ? fmtProbability(row.calibrated_hit_probability) : "—");
  setText(
    "drawerPitcher",
            ...
            ${bestPitcherHitters.length ? bestPitcherHitters.map((row, index) => {
              ...
              return `
                <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}">
                  ...
                  <td class="num">${row.calibrated_hit_probability != null ? `<span class="probability-badge">${fmtProbability(row.calibrated_hit_probability)}</span>` : "—"}</td>
