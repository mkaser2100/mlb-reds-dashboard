...
console.info("MLB Hit Lab app-v4 loaded: v3-feature-importance-ui-v1");
  ...
  return `
    <div class="v3-group-list">
      ${rows.map((item) => {
        ...
        const label = String(item.feature_group || "Other")
          .replaceAll("_", " ")
          .replace(/\b\w/g, (letter) => letter.toUpperCase());
        const pct = v3ContributionPct(item.contribution_pct);
        const rawPct = Math.abs(Number(item.contribution_pct || 0));
        const width = Math.max(4, Math.min(100, rawPct <= 1 ? rawPct * 100 : rawPct));
...
async function loadMlbHitBoardData() {
  try {
    ...
    const rawV3Rows = (v3Result.data || [])
      .filter((row) => row.batter_name || row.full_name);
    const latestPredictionDate = rawV3Rows[0]?.prediction_run_date || null;
...
const MP_FEATURE_GROUPS = {
  ...
  availability: {
    ...
    description: "Signals that tell the model whether advanced matchup data is complete and reliable."
  }
};
