...
console.info("MLB Hit Lab app-v4 loaded: market-edge-table-parity-v7");
...
function marketEdgeValueClass(edgePct) {
  const n = Number(edgePct || 0);
  if (n >= 5) return "market-edge-value-play";
  if (n >= 3) return "market-edge-value-watch";
  if (n >= 1) return "market-edge-value-fair";
  return "market-edge-value-efficient";
}

function renderMarketEdgeValuePill(row) {
  return `
    <span class="primary-why-wrap market-edge-value-wrap">
      <span class="why-pill primary-why-pill market-edge-value-pill ${marketEdgeValueClass(row?.edge_pct)}">
