...
console.info("MLB Hit Lab app-v4 loaded: v3-definitive-lookup-v9");
...
const V3_DRAWER_EXPLANATION_SELECT = "prediction_id,model_run_id,target_name,prediction_run_date,game_pk,player_id,rank_overall,headline,explanation_text,positive_tags,negative_tags,generation_method,generation_status,contribution_model_name,contribution_model_version,contribution_model_status,confidence_bucket,rank_team,calculation_method,top_positive_drivers,top_risk_factors,group_contributions,all_features,feature_count";
...
function v3DateKey(value) {
  if (!value) return "";
  return String(value).slice(0, 10);
}

function latestV3PredictionDate(rows = []) {
  return (rows || []).reduce((latest, row) => {
    const date = v3DateKey(row?.prediction_run_date);
    return date && (!latest || date > latest) ? date : latest;
  }, "");
}

function v3ExplanationPlayerGameDateKey(row) {
  const date = v3DateKey(row?.prediction_run_date);
  const gamePk = row?.game_pk ?? "";
  const playerId = row?.player_id ?? "";
  return date && gamePk !== "" && playerId !== ""
    ? `${date}|${String(gamePk)}|${String(playerId)}`
    : "";
}

function v3ExplanationPlayerDateKey(row) {
  const date = v3DateKey(row?.prediction_run_date);
  const playerId = row?.player_id ?? "";
  return date && playerId !== "" ? `${date}|${String(playerId)}` : "";
}

function v3JsonArray(value) {
  ...
  if (typeof value === "string") {
    try {
      ...
      console.warn("Unable to parse V3 contribution array:", err);
    }
  }
...
const V3_DIAGNOSTICS = true;
let v3LastDiagnostics = null;

function v3DiagnosticSnapshot(row, candidates = [], selected = null, stage = "lookup") {
  const snapshot = {
    stage,
    boardRow: {
      prediction_id: row?.prediction_id ?? null,
      model_run_id: row?.model_run_id ?? null,
      prediction_run_date: v3DateKey(row?.prediction_run_date),
      game_pk: row?.game_pk ?? null,
      player_id: row?.player_id ?? null,
      batter_name: row?.batter_name || row?.full_name || null,
      rank_overall: row?.rank_overall ?? null,
      existing_positive_count: v3JsonArray(row?.drawer_top_positive_drivers).length,
      existing_risk_count: v3JsonArray(row?.drawer_top_risk_factors).length,
      existing_group_count: v3JsonArray(row?.drawer_group_contributions).length,
      contribution_status: row?.drawer_contribution_data_status ?? null
    },
    candidates: (candidates || []).map((item) => ({
      prediction_id: item?.prediction_id ?? null,
      model_run_id: item?.model_run_id ?? null,
      prediction_run_date: v3DateKey(item?.prediction_run_date),
      game_pk: item?.game_pk ?? null,
      player_id: item?.player_id ?? null,
      rank_overall: item?.rank_overall ?? null,
      positive_count: v3JsonArray(item?.top_positive_drivers ?? item?.drawer_top_positive_drivers).length,
      risk_count: v3JsonArray(item?.top_risk_factors ?? item?.drawer_top_risk_factors).length,
      group_count: v3JsonArray(item?.group_contributions ?? item?.drawer_group_contributions).length
    })),
    selected: selected ? {
      prediction_id: selected?.prediction_id ?? null,
      model_run_id: selected?.model_run_id ?? null,
      prediction_run_date: v3DateKey(selected?.prediction_run_date),
      game_pk: selected?.game_pk ?? null,
      player_id: selected?.player_id ?? null,
      positive_count: v3JsonArray(selected?.top_positive_drivers ?? selected?.drawer_top_positive_drivers).length,
      risk_count: v3JsonArray(selected?.top_risk_factors ?? selected?.drawer_top_risk_factors).length,
      group_count: v3JsonArray(selected?.group_contributions ?? selected?.drawer_group_contributions).length
    } : null
  };

  v3LastDiagnostics = snapshot;

  if (V3_DIAGNOSTICS) {
    console.groupCollapsed(`[V3 Drawer] ${stage}`);
    console.log("Board row", snapshot.boardRow);
    console.table(snapshot.candidates);
    console.log("Selected explanation", snapshot.selected);
    console.groupEnd();
  }
...
function v3DiagnosticPanel(snapshot, hydratedRow = null) {
  if (!V3_DIAGNOSTICS || !snapshot) return "";

  const board = snapshot.boardRow || {};
  const selected = snapshot.selected || {};
  const hydrated = {
    positive_count: v3JsonArray(hydratedRow?.drawer_top_positive_drivers).length,
    risk_count: v3JsonArray(hydratedRow?.drawer_top_risk_factors).length,
    group_count: v3JsonArray(hydratedRow?.drawer_group_contributions).length,
    status: hydratedRow?.drawer_contribution_data_status || "unknown"
  };

  const candidateRows = (snapshot.candidates || []).map((item, index) => `
    <tr>
      <td>${index + 1}</td>
      <td>${escapeHtml(item.prediction_id ?? "—")}</td>
      <td>${escapeHtml(item.game_pk ?? "—")}</td>
      <td>${escapeHtml(item.player_id ?? "—")}</td>
      <td>${escapeHtml(item.positive_count)}</td>
      <td>${escapeHtml(item.risk_count)}</td>
      <td>${escapeHtml(item.group_count)}</td>
    </tr>
  `).join("");

  return `
    <details class="v3-diagnostics-panel" open>
      <summary>V3 Developer Diagnostics</summary>
      <div class="v3-diagnostics-grid">
        <div><span>Board player</span><strong>${escapeHtml(board.player_id ?? "—")}</strong></div>
        <div><span>Board game</span><strong>${escapeHtml(board.game_pk ?? "—")}</strong></div>
        <div><span>Board prediction</span><strong>${escapeHtml(board.prediction_id ?? "—")}</strong></div>
        <div><span>Board date</span><strong>${escapeHtml(board.prediction_run_date || "—")}</strong></div>
        <div><span>Selected game</span><strong>${escapeHtml(selected.game_pk ?? "—")}</strong></div>
        <div><span>Candidates</span><strong>${escapeHtml(snapshot.candidates?.length || 0)}</strong></div>
        <div><span>Hydrated positives</span><strong>${escapeHtml(hydrated.positive_count)}</strong></div>
        <div><span>Hydrated risks</span><strong>${escapeHtml(hydrated.risk_count)}</strong></div>
        <div><span>Hydrated groups</span><strong>${escapeHtml(hydrated.group_count)}</strong></div>
        <div><span>Status</span><strong>${escapeHtml(hydrated.status)}</strong></div>
      </div>
      <div class="v3-diagnostics-table-wrap">
        <table class="v3-diagnostics-table">
          <thead>
            <tr><th>#</th><th>Prediction</th><th>Game</th><th>Player</th><th>+</th><th>-</th><th>Groups</th></tr>
          </thead>
          <tbody>${candidateRows || `<tr><td colspan="7">No explanation candidates returned.</td></tr>`}</tbody>
        </table>
      </div>
    </details>
  `;
...
function hydrateV3DrawerExplanation(row, explanation) {
  if (!row || !explanation) return row;

  const positiveDrivers = v3JsonArray(explanation.top_positive_drivers ?? explanation.drawer_top_positive_drivers);
  const riskFactors = v3JsonArray(explanation.top_risk_factors ?? explanation.drawer_top_risk_factors);
  const groupContributions = v3JsonArray(explanation.group_contributions ?? explanation.drawer_group_contributions);
  const allFeatures = v3JsonArray(explanation.all_features ?? explanation.drawer_all_features);

  return {
    ...row,
    drawer_explanation_headline: explanation.headline ?? explanation.drawer_explanation_headline ?? row.drawer_explanation_headline ?? null,
    drawer_explanation_text: explanation.explanation_text ?? explanation.drawer_explanation_text ?? row.drawer_explanation_text ?? null,
    drawer_positive_tags: v3JsonArray(explanation.positive_tags ?? explanation.drawer_positive_tags),
    drawer_negative_tags: v3JsonArray(explanation.negative_tags ?? explanation.drawer_negative_tags),
    drawer_explanation_method: explanation.generation_method ?? explanation.drawer_explanation_method ?? null,
    drawer_explanation_status: explanation.generation_status ?? explanation.drawer_explanation_status ?? null,
    drawer_contribution_model_name: explanation.contribution_model_name ?? explanation.drawer_contribution_model_name ?? null,
    drawer_contribution_model_version: explanation.contribution_model_version ?? explanation.drawer_contribution_model_version ?? null,
    drawer_contribution_model_status: explanation.contribution_model_status ?? explanation.drawer_contribution_model_status ?? null,
    drawer_confidence_bucket: explanation.confidence_bucket ?? explanation.drawer_confidence_bucket ?? null,
    drawer_rank_team: explanation.rank_team ?? explanation.drawer_rank_team ?? null,
    drawer_calculation_method: explanation.calculation_method ?? explanation.drawer_calculation_method ?? null,
    drawer_top_positive_drivers: positiveDrivers,
    drawer_top_risk_factors: riskFactors,
    drawer_group_contributions: groupContributions,
    drawer_all_features: allFeatures,
    drawer_feature_count: Number(explanation.feature_count ?? explanation.drawer_feature_count ?? allFeatures.length ?? 0),
    drawer_contribution_data_status:
      positiveDrivers.length || riskFactors.length || groupContributions.length ? "loaded" : "empty"
  };
}

function mergeV3DrawerExplanations(rows, explanationRows = []) {
  const byPredictionId = new Map();
  const byPlayerGameDate = new Map();
  ...
  (explanationRows || []).forEach((explanation) => {
    ...
    const playerGameDateKey = v3ExplanationPlayerGameDateKey(explanation);
    if (playerGameDateKey) byPlayerGameDate.set(playerGameDateKey, explanation);

    const playerDateKey = v3ExplanationPlayerDateKey(explanation);
    if (playerDateKey) byPlayerDate.set(playerDateKey, explanation);
  });

  return (rows || []).map((row) => {
    const predictionId = row?.prediction_id;
    const explanation =
      (predictionId !== null && predictionId !== undefined
        ? byPredictionId.get(String(predictionId))
        : null) ||
      byPlayerGameDate.get(v3ExplanationPlayerGameDateKey(row)) ||
      byPlayerDate.get(v3ExplanationPlayerDateKey(row));

    if (!explanation) {
      return {
        ...row,
        drawer_contribution_data_status: "missing"
      };
    }

    return hydrateV3DrawerExplanation(row, explanation);
  });
}

async function loadV3DrawerExplanations(predictionDate) {
  if (!predictionDate) {
    console.warn("V3 drawer explanations were not requested because no prediction date was resolved.");
    return [];
  }
  ...
  try {
    const { data, error } = await client
      ...
      .eq("prediction_run_date", predictionDate)
      .order("rank_overall", { ascending: true });

    if (error) throw error;

    const rows = data || [];
    console.info("Loaded V3 drawer explanations", {
      predictionDate,
      rows: rows.length
    });
    return rows;
  } catch (err) {
    console.error("Error loading V3 drawer explanations:", {
      predictionDate,
...
function ensureV3DrawerExplanationStyles() {
  if (document.getElementById("v3DrawerExplanationStyles")) return;

  const style = document.createElement("style");
  style.id = "v3DrawerExplanationStyles";
  style.textContent = `
    .v3-explanation-headline {
      margin-top: 14px;
      color: var(--text);
      font-size: 15px;
      font-weight: 800;
    ...
    .v3-diagnostics-grid strong {
      display: block;
      margin-top: 5px;
      word-break: break-word;
    }

    .v3-diagnostics-table-wrap {
      overflow-x: auto;
      padding: 0 16px 16px;
    }

    .v3-diagnostics-table {
      width: 100%;
      border-collapse: collapse;
      font-size: 12px;
    }

    .v3-diagnostics-table th,
    .v3-diagnostics-table td {
      padding: 8px;
...
async function loadRedsGameBoardData() {
  try {
    ...
    const rawRedsMlRows = (mlResult.data || [])
      .filter((row) => row.batter_name || row.full_name);
    const redsPredictionDate = rawRedsMlRows[0]?.prediction_run_date || null;

    if (!v3DrawerExplanationRows.length || (redsPredictionDate && !v3DrawerExplanationRows.some((row) => row.prediction_run_date === redsPredictionDate))) {
      v3DrawerExplanationRows = await loadV3DrawerExplanations(redsPredictionDate);
    }

    redsMlRows = mergeV3DrawerExplanations(rawRedsMlRows, v3DrawerExplanationRows)
...
function renderRedsMlRows(rows) {
  ...
  return rows.map((row, index) => `
    <tr class="clickable-row" data-reds-v3-player-id="${row.player_id}" data-reds-v3-game-pk="${row.game_pk ?? ""}">
...
function renderRedsClassicRows(rows) {
  ...
  return rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-reds-classic-player-id="${row.player_id}" data-reds-classic-game-pk="${row.game_pk ?? ""}">
        <td class="rank"><span class="rank-badge">${index + 1}</span></td>
        <td>
          <div class="player-cell">
...
function openRedsV3Drawer(playerId, gamePk = null) {
  ...
  const row =
    (normalizedGamePk
      ? redsMlRows.find(
          (x) =>
            String(x.player_id ?? "") === normalizedPlayerId &&
            String(x.game_pk ?? "") === normalizedGamePk
        )
      : null) ||
    redsMlRows.find((x) => String(x.player_id ?? "") === normalizedPlayerId);

  if (row) openV3MlbDrawer(row);
}

function openRedsClassicDrawer(playerId, gamePk = null) {
  const priorMode = mlbBoardMode;
  const priorRows = mlbClassicRows;
  mlbBoardMode = "classic";
  mlbClassicRows = redsClassicRows;
  openMlbDrawer(playerId, gamePk);
...
function v3DriverRows(items, direction) {
  ...
  return `
    <div class="v3-driver-list">
      ${rows.map((item) => {
        const label = item.display_name || item.feature_name || "Model signal";
        ...
        return `
          <article class="v3-driver-card ${direction}">
            ...
            <div class="v3-driver-copy">
              <div class="v3-driver-title">${escapeHtml(label)}</div>
              ${explanation ? `<div class="v3-driver-explanation">${escapeHtml(explanation)}</div>` : ""}
            </div>
            <div class="v3-driver-pct">${escapeHtml(pct)}</div>
          </article>
        `;
      }).join("")}
    </div>
...
async function loadV3DrawerExplanationForRow(row) {
  const predictionDate = v3DateKey(row?.prediction_run_date);
  const playerId = row?.player_id;
  const gamePk = row?.game_pk;
  const predictionId = row?.prediction_id;

  if (!predictionDate || playerId === null || playerId === undefined) {
    v3DiagnosticSnapshot(row, [], null, "lookup skipped: missing date or player");
    return null;
  }

  const finish = (candidates, explanation, stage) => {
    const snapshot = v3DiagnosticSnapshot(row, candidates, explanation, stage);
    return explanation
      ? { ...explanation, __v3DiagnosticSnapshot: snapshot }
      : null;
  };

  try {
    // Strongest contract: prediction_id uniquely identifies the explanation row.
    if (predictionId !== null && predictionId !== undefined && predictionId !== "") {
      const { data, error } = await client
        .from("v_mlb_v3_player_drawer_explanations")
        .select("*")
      ...
      if (!error && data?.length) {
        const exact = data.find(
          (item) => String(item?.prediction_id ?? "") === String(predictionId)
        ) || data[0];

        return finish(data, exact, "prediction_id lookup: matched");
      }

      if (error) {
        console.warn("V3 prediction_id lookup failed; continuing with composite lookup", {
          predictionId,
    ...
    // Second contract: player + prediction date + game.
    ...
    if (gamePk !== null && gamePk !== undefined && gamePk !== "") {
      exactQuery = exactQuery.eq("game_pk", gamePk);
    }
    ...
    if (!error && data?.length) {
      const exactGame = data.find(
        (item) => String(item?.game_pk ?? "") === String(gamePk ?? "")
      ) || data[0];

      return finish(data, exactGame, "player + date + game lookup: matched");
    }

    if (error) {
      console.warn("V3 composite lookup failed; continuing with player/date fallback", {
        predictionDate,
        playerId,
        gamePk,
        message: error.message
    ...
    // Final fallback supports rows where the board game key changed but the player/date did not.
    ...
    const exactPrediction = candidates.find(
      (item) =>
        predictionId !== null &&
        predictionId !== undefined &&
        String(item?.prediction_id ?? "") === String(predictionId)
    );
    const exactGame = candidates.find(
      (item) => String(item?.game_pk ?? "") === String(gamePk ?? "")
    );
    const explanation = exactPrediction || exactGame || candidates[0] || null;

    const matchMethod = exactPrediction
      ? "prediction_id"
      : exactGame
        ? "game_pk"
        : explanation
          ? "first player/date candidate"
          : "no candidate";

    return finish(
      candidates,
      explanation,
      `player + date fallback: matched by ${matchMethod}`
    );
  } catch (err) {
    v3DiagnosticSnapshot(
      row,
      [],
      null,
      `lookup error: ${err?.message || String(err)}`
    );
    console.error("Error loading direct V3 drawer explanation:", {
      predictionDate,
      predictionId,
      playerId,
      gamePk,
...
async function openV3MlbDrawer(row) {
  ensureV3DrawerExplanationStyles();
  let drawerDiagnostics = v3DiagnosticSnapshot(row, [], null, "drawer opened");
  ...
  const explanationHeadline = row.drawer_explanation_headline || "";
  const explanation = row.drawer_explanation_text || row.explanation_text || row.explanation || "No ML explanation available yet.";
  const gameTime = formatEasternGameTime(row.game_time_utc);
  const confidenceClass = v3ConfidenceClass(confidence);
  let positiveDrivers = v3JsonArray(row.drawer_top_positive_drivers);
  let riskFactors = v3JsonArray(row.drawer_top_risk_factors);
  let groupContributions = v3JsonArray(row.drawer_group_contributions);
  let hasExactContributions = positiveDrivers.length || riskFactors.length || groupContributions.length;

  if (!hasExactContributions && !row.__drawerDirectLookupAttempted) {
    const directExplanation = await loadV3DrawerExplanationForRow(row);
    drawerDiagnostics = directExplanation?.__v3DiagnosticSnapshot || v3LastDiagnostics || drawerDiagnostics;
    row = { ...row, __drawerDirectLookupAttempted: true };

    if (directExplanation) {
      row = hydrateV3DrawerExplanation(row, directExplanation);
      positiveDrivers = v3JsonArray(row.drawer_top_positive_drivers);
      riskFactors = v3JsonArray(row.drawer_top_risk_factors);
      groupContributions = v3JsonArray(row.drawer_group_contributions);
      hasExactContributions = Boolean(
        positiveDrivers.length || riskFactors.length || groupContributions.length
      );
    }
  }
  ...
  body.innerHTML = `
    ...
    <section class="drawer-section v3-section">
      ...
      ${positiveDrivers.length
        ? v3DriverRows(positiveDrivers, "positive")
        : `
          <div class="v3-driver-unavailable" role="status">
            <strong>V3 driver data unavailable</strong>
            <span>The lookup completed but no usable feature-contribution payload was hydrated.</span>
            <small class="v3-driver-diagnostic-summary">
              ${escapeHtml(drawerDiagnostics?.stage || "No lookup diagnostic was recorded.")}
            </small>
          </div>
        `}
      ${!hasExactContributions && explanationHeadline ? `<div class="v3-explanation-headline">${escapeHtml(explanationHeadline)}</div>` : ""}
      ${!hasExactContributions ? `<p class="drawer-explanation v3-explanation">${escapeHtml(explanation)}</p>` : ""}
    </section>

    ${v3DiagnosticPanel(drawerDiagnostics, row)}

    ${riskFactors.length ? `
      <section class="drawer-section v3-section">
        <div class="drawer-section-title">Main Risks</div>
        ${v3DriverRows(riskFactors, "negative")}
      </section>
    ` : v3RiskPills(row) ? `
      <section class="drawer-section v3-section">
        <div class="drawer-section-title">Main Risks</div>
    ...
    ${groupContributions.length ? `
      <section class="drawer-section v3-section">
        <div class="drawer-section-title">Model Breakdown</div>
        <div class="v3-breakdown-note">Share of this player’s total feature impact by signal group.</div>
        ${v3GroupRows(groupContributions)}
      </section>
    ` : ""}
...
function openMlbDrawer(playerId, gamePk = null) {
  ...
  const baseRow =
    (normalizedGamePk
      ...
      : null) ||
    activeMlbRows.find((x) => String(x.player_id ?? "") === normalizedPlayerId);
...
function normalizeV3HitRow(row) {
  ...
  return {
    ...
    drawer_explanation_text: drawerExplanationText,
    ...
    drawer_top_positive_drivers: v3JsonArray(row.drawer_top_positive_drivers),
    drawer_top_risk_factors: v3JsonArray(row.drawer_top_risk_factors),
    drawer_group_contributions: v3JsonArray(row.drawer_group_contributions),
    drawer_all_features: v3JsonArray(row.drawer_all_features),
    drawer_feature_count: Number(row.drawer_feature_count || 0),
    drawer_contribution_data_status: row.drawer_contribution_data_status || null,
...
async function openMarketEdgeDrawer(playerId) {
  ...
  try {
    ...
    if (detailRow) {
      const withDrawerExplanation = mergeV3DrawerExplanations(
        [{ ...detailRow, ...marketRow }],
        v3DrawerExplanationRows
      )[0];
      drawerRow = withDrawerExplanation || { ...detailRow, ...marketRow };
    }
  } catch (err) {
...
async function loadMlbHitBoardData() {
  try {
    ...
    const allV3Rows = (v3Result.data || [])
      .filter((row) => row.batter_name || row.full_name);
    const latestPredictionDate = latestV3PredictionDate(allV3Rows);

    if (!latestPredictionDate) {
      throw new Error("The V3 daily view returned no prediction_run_date.");
    }

    // The daily view can temporarily expose more than one prediction date.
    // Keep the board and drawer explanations locked to the same latest date.
    const rawV3Rows = allV3Rows.filter(
      (row) => v3DateKey(row.prediction_run_date) === latestPredictionDate
    );

    v3DrawerExplanationRows = await loadV3DrawerExplanations(latestPredictionDate);

    const baseRows = mergeV3DrawerExplanations(rawV3Rows, v3DrawerExplanationRows)
      .map(normalizeV3HitRow)
      .sort((a, b) => Number(a.rank_overall || 9999) - Number(b.rank_overall || 9999));

    const contributionMatches = baseRows.filter(
      (row) => row.drawer_contribution_data_status === "loaded"
    ).length;

    console.info("Prepared latest V3 board", {
      predictionDate: latestPredictionDate,
      boardRows: baseRows.length,
      explanationRows: v3DrawerExplanationRows.length,
      contributionMatches
    });
        ...
        <table>
          ...
          <tbody>
            ${top25.length ? top25.map((row, index) => `
              <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}" data-mlb-game-pk="${row.game_pk ?? ""}">
...
function wireEvents() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (row.dataset.redsV3PlayerId) {
      openRedsV3Drawer(row.dataset.redsV3PlayerId, row.dataset.redsV3GamePk);
      return;
    }

    if (row.dataset.redsClassicPlayerId) {
      openRedsClassicDrawer(row.dataset.redsClassicPlayerId, row.dataset.redsClassicGamePk);
      return;
    }
    ...
    if (row.dataset.mlbPlayerId) {
      openMlbDrawer(row.dataset.mlbPlayerId, row.dataset.mlbGamePk);
      return;
    }
...
const MODEL_PERFORMANCE_FIX_VERSION = "model-performance-redesign-v6-dedicated-v3-view";
...
function mpNormalizeWindowLabel(value, row = {}) {
  const raw = String(value || mpField(row, ["range_label", "window_label", "time_window", "period", "lookback_label"], "Season"));
  const lower = raw.toLowerCase();
  const days = Number(mpField(row, ["window_days", "days", "lookback_days"], NaN));

  if (lower.includes("season") || days === 9999) return "Season";
  if (lower.includes("last 30") || days === 30) return "Last 30";
  if (lower.includes("last 14") || days === 14) return "Last 14";
  if (lower.includes("last 7") || days === 7) return "Last 7";
  if (lower === "last30") return "Last 30";
  if (lower === "last14") return "Last 14";
  if (lower === "last7") return "Last 7";
...
function mpScorecardSampleNote(row, metric, isBest) {
  ...
  if (model === "V3" && days !== null && days < 10) {
    return `<span class="scorecard-caution" title="Small evaluated sample">⚠ ${days} days</span>`;
  }
...
function mpSelectedScorecardSmallSampleNote() {
  const rows = mpScorecardRowsForSelectedWindow();
  const v3 = rows.find((row) => mpModel(row) === "V3");
  const v3Days = v3 ? mpScorecardDays(v3) : null;

  if (v3Days !== null && v3Days < 10) {
    return `<div class="scorecard-footnote">⚠ V3 ML has only ${fmtNum(v3Days)} evaluated days in this window, so perfect Top Pick rates should be treated as directional until the sample grows.</div>`;
  }
