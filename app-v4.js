...
function openMlbDrawer(playerId) {
  ...
  setText(
    "drawerPlayerSub",
    `${row.team_name || "MLB"} · Player ID ${playerId} · V3 hit probability model`
  );

  setText("drawerMatchupScore", row.hit_probability_pct != null ? `${fmtDecimal(row.hit_probability_pct, 1)}%` : fmtDecimal(row.matchup_score, 1));
  setText("drawerV2PickScore", row.predicted_probability != null ? `${fmtProbabilityPct(row)} ML` : (row.model_v2_score != null ? fmtDecimal(row.model_v2_score, 1) : "—"));
  setText(
    "drawerPitcher",
  ...
  setText("drawerMatchupExplanation", row.explanation_text || matchup.explanation || "No ML explanation available yet.");
  renderScoreBreakdown(matchup);
...
function renderWhyPills(row) {
  ...
  if (/recent hit|hot recently|recent form/i.test(text)) pills.push("Hot Batter");
  if (/split is favorable|favorable split|batter split/i.test(text)) pills.push("Great Split");
  if (/pitcher allows|allowed traffic|high batting average|weak pitcher/i.test(text)) pills.push("Pitcher Edge");
  if (/risk|tougher|strong, adding risk/i.test(text)) pills.push("Risk Note");

  if (!pills.length) pills.push(row?.confidence_bucket === "high" ? "ML Edge" : "Model Read");

  return pills.slice(0, 3).map((pill) => `<span class="why-pill">${pill}</span>`).join("");
}
...
function renderMlbHitBoardPage(error = null) {
  ...
  content.innerHTML = `
    <section class="hero-grid v3-hero-grid">
      <article class="insight-card primary-insight">
        <div class="card-topline">
          <span>Today's Top Pick</span>
          <span>${modelStatusText()}</span>
        </div>
        <div class="hero-player">${topPick?.batter_name || topPick?.full_name || "Loading ML board..."}</div>
        <div class="hero-score">${topPick ? `${fmtProbabilityPct(topPick)} hit probability` : "Hit Probability —"}</div>
        <p>${topPickNarrative(topPick)}</p>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">🎯</div>
        <div class="metric-label">Top 10 Avg</div>
        <div class="metric-value">${top10Avg != null ? `${top10Avg.toFixed(1)}%` : "—"}</div>
        <div class="metric-sub">ML hit probability</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">🧠</div>
        <div class="metric-label">Model</div>
        <div class="metric-value">ML Prediction</div>
        <div class="metric-sub">${v3ModelRegistry?.model_version || "V3 candidate"}</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">✅</div>
        <div class="metric-label">Actuals</div>
        <div class="metric-value">${v3ActualsStatus?.actual_loaded_rows ? fmtNum(v3ActualsStatus.actual_loaded_rows) : "0"}</div>
        <div class="metric-sub">${actualsStatusText()}</div>
      </article>
    </section>
    ...
    <section class="control-deck performance-window-deck v3-control-deck">
      <div class="control-group">
        ...
        <div class="segmented">
          <button class="segment active" type="button">⭐ ML Prediction</button>
          <button class="segment legacy-segment" type="button" title="Legacy score retained for model lab comparison">Classic Score</button>
        </div>
      </div>
