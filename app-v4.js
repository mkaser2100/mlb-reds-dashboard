...
console.info("MLB Hit Lab app-v4 loaded: reds-v3-game-board-v2");
...
async function loadRedsGameV3Data() {
  const gamePk = matchupRows.find((row) => row?.game_pk)?.game_pk;
  const gameDate = matchupRows.find((row) => row?.game_date)?.game_date;
  ...
  try {
    ...
    redsGameV3Rows = (data || [])
      ...
      .sort((a, b) => Number(b.hit_probability_pct || b.matchup_score || 0) - Number(a.hit_probability_pct || a.matchup_score || 0));
  } catch (err) {
    console.error("Error loading Reds game V3 hit probabilities:", err);
    redsGameV3Rows = [];
  }
}
...
function renderTable() {
  const v3Rows = redsGameV3Rows
    .filter((row) => {
      const q = searchTerm.trim().toLowerCase();
      if (!q) return true;
      return String(row.batter_name || row.full_name || "").toLowerCase().includes(q) ||
        String(row.team_name || "").toLowerCase().includes(q);
    })
    .slice()
    .sort((a, b) => Number(b.hit_probability_pct || b.matchup_score || 0) - Number(a.hit_probability_pct || a.matchup_score || 0));

  if (v3Rows.length) {
    const html = v3Rows.map((row, index) => `
      <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}">
        ...
        <td><span class="confidence-badge ${v3ConfidenceClass(row.confidence_bucket)}">${titleCase(row.confidence_bucket || "model")}</span></td>
    ...
    setText("statusPill", `${v3Rows.length} game hitters scored`);
    setText("lastRefresh", `V3 ML · Reds + opponent`);
    return;
  }
  ...
  if (!rows.length) {
    setHtml("hittersTableBody", `
      <tr>
        <td colspan="8" class="empty-state">No hitters match your search.</td>
      </tr>
    `);
    return;
  }
  ...
  const html = rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-player-id="${row.player_id}">
        ...
        <td><span class="confidence-badge confidence-medium">Classic</span></td>
        <td class="why-cell">${matchup ? `<span class="why-pill primary-why-pill reason-read"><span class="why-emoji">📈</span><span>Matchup Score</span></span>` : "—"}</td>
        <td>
...
function openMlbDrawer(playerId) {
  const activeMlbRows = mlbBoardMode === "classic" ? mlbClassicRows : [...redsGameV3Rows, ...mlbRows];
