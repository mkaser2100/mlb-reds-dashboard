...
console.info("MLB Hit Lab app-v4 loaded: github-performance-normalized-option1-v1");
...
let appModelScorecardRows = [];
let appRollingPerformanceRows = [];
let appScoreBucketRows = [];
let appFeatureSignalRows = [];
let modelPerformanceView = "all";
let modelPerformanceRange = "season";
let marketEdgeRows = [];
...
async function fetchPerformanceView(viewName, orderBy = null) {
  ...
  const { data, error } = await query;
  if (error) {
    console.error(`Performance view failed: ${viewName}`, error);
    return [];
  }
  return data || [];
}
...
async function loadPerformanceData() {
  const [scorecardRows, rollingRows, bucketRows, featureRows] = await Promise.all([
    fetchPerformanceView("v_mlb_app_model_scorecard", { column: "model_key", ascending: true }),
    fetchPerformanceView("v_mlb_app_rolling_model_performance", { column: "window_days", ascending: true }),
    fetchPerformanceView("v_mlb_app_model_score_buckets", { column: "bucket_order", ascending: true }),
    fetchPerformanceView("v_mlb_app_feature_signals", { column: "model_key", ascending: true })
  ]);

  appModelScorecardRows = scorecardRows;
  appRollingPerformanceRows = rollingRows;
  appScoreBucketRows = bucketRows;
  appFeatureSignalRows = featureRows;

  console.info("Model performance rows loaded", {
    scorecard: appModelScorecardRows.length,
    rolling: appRollingPerformanceRows.length,
    buckets: appScoreBucketRows.length,
    features: appFeatureSignalRows.length
  });
...
function modelDisplayName(modelKey) {
  const names = {
    ...
    v2_proxy: "V2 Recommended",
    v3_ml: "V3 ML"
  };
  return names[modelKey] || modelKey || "Model";
}

function modelShortName(modelKey) {
  const names = {
    v1_classic: "V1",
    v2_proxy: "V2",
    v3_ml: "V3"
...
function modelOrder(modelKey) {
  return { v1_classic: 1, v2_proxy: 2, v3_ml: 3 }[modelKey] || 99;
}

function selectedModelKeys() {
  return modelPerformanceView === "all"
    ? ["v1_classic", "v2_proxy", "v3_ml"]
    : [modelPerformanceView];
}

function modelPill(modelKey) {
  return `<span class="model-chip model-${escapeHtml(modelKey)}">${escapeHtml(modelShortName(modelKey))}</span>`;
}
...
function performanceWinnerClass(value, values) {
  const nums = values.map(Number).filter(Number.isFinite);
  if (!nums.length || !Number.isFinite(Number(value))) return "";
  return Number(value) === Math.max(...nums) ? " is-winner" : "";
}

function renderModelPerformanceControls() {
  const modelOptions = [
    ["all", "All"],
    ["v1_classic", "V1"],
    ["v2_proxy", "V2"],
  ...
  return `
    <section class="control-deck performance-window-deck model-performance-controls">
      <div class="control-group">
        <div class="control-label">Model View</div>
        <div class="segmented" id="modelPerformanceViewButtons">
          ${modelOptions.map(([key, label]) => `
            <button class="segment ${modelPerformanceView === key ? "active" : ""}" data-model-performance-view="${key}" type="button">${label}</button>
          `).join("")}
        </div>
      </div>

      <div class="control-group">
        <div class="control-label">Scorecard Range</div>
        <div class="segmented" id="modelPerformanceRangeButtons">
          ${rangeOptions.map(([key, label]) => `
            <button class="segment ${modelPerformanceRange === key ? "active" : ""}" data-model-performance-range="${key}" type="button">${label}</button>
          `).join("")}
        </div>
      </div>
      ...
      <div class="control-group grow">
        ...
        <div class="sort-pill">Cards 2 and 3 always compare all models side-by-side</div>
      </div>
    </section>
  `;
}
...
function renderModelScorecardCard() {
  ...
  const rows = appModelScorecardRows
    .filter((row) => row.range_key === modelPerformanceRange && keys.includes(row.model_key))
    .sort((a, b) => modelOrder(a.model_key) - modelOrder(b.model_key));

  if (!rows.length) {
    return `
      <section class="board-card performance-card">
        <div class="board-header">
          <div><div class="eyebrow">Cross-Model Bucket Performance</div><h2>Model Scorecard</h2></div>
        </div>
  ...
  const cols = [
    ["top1_hit_rate_pct", "Top Pick"],
    ["top5_hit_rate_pct", "Top 5"],
    ["top10_hit_rate_pct", "Top 10"],
    ["top20_hit_rate_pct", "Top 20"],
    ["top25_hit_rate_pct", "Top 25"],
    ["overall_hit_rate_pct", "Overall"]
  ];

  return `
    <section class="board-card performance-card model-scorecard-card">
      <div class="board-header">
        <div>
          <div class="eyebrow">Cross-Model Bucket Performance</div>
          <h2>Model Scorecard</h2>
        </div>
        <div class="board-meta"><span>${escapeHtml(rows[0]?.range_label || "Season")}</span><span>${fmtNum(rows[0]?.days)} days</span></div>
      </div>
      <div class="table-wrap performance-table-wrap">
        <table class="performance-table model-scorecard-table">
          ...
          <tbody>
            ${rows.map((row) => `
              <tr>
                <td>${modelPill(row.model_key)} <strong>${escapeHtml(row.model_name || modelDisplayName(row.model_key))}</strong></td>
                ${cols.map(([field]) => {
                  const values = rows.map((r) => r[field]);
                  return `<td class="num${performanceWinnerClass(row[field], values)}">${fmtPctValue(row[field])}</td>`;
                }).join("")}
              </tr>
            `).join("")}
          </tbody>
...
function renderRollingPerformanceCard() {
  const windows = [...new Map(appRollingPerformanceRows
    .sort((a, b) => Number(a.window_days || 0) - Number(b.window_days || 0))
    .map((row) => [String(row.window_days), row])).values()];
  ...
  const modelKeys = ["v1_classic", "v2_proxy", "v3_ml"];

  if (!windows.length) {
    return `
      <section class="board-card performance-card full-width-card">
        <div class="board-header"><div><div class="eyebrow">Rolling Performance</div><h2>Recent Bucket Hit Rates</h2></div></div>
        <div class="empty-state">No rolling model performance rows available.</div>
      </section>
    `;
  }
  ...
  return `
    <section class="board-card performance-card full-width-card">
      <div class="board-header">
        ...
        <div class="board-meta"><span>All models shown</span><span>Winner highlighted</span></div>
      </div>
      <div class="table-wrap performance-table-wrap">
        <table class="performance-table rolling-pivot-table">
          <thead>
            <tr>
              <th rowspan="2">Window</th>
              ${buckets.map(([, label]) => `<th class="group-head" colspan="3">${label}</th>`).join("")}
            </tr>
            <tr>
              ${buckets.map(() => modelKeys.map((key) => `<th class="num model-head model-${key}">${modelShortName(key)}</th>`).join("")).join("")}
            </tr>
          </thead>
          <tbody>
            ${windows.map((windowRow) => {
              const rowsForWindow = appRollingPerformanceRows.filter((row) => String(row.window_days) === String(windowRow.window_days));
              return `
                <tr>
                  <td><strong>${escapeHtml(windowRow.window_label || `Last ${windowRow.window_days}`)}</strong><div class="player-sub">${fmtNum(windowRow.days_in_window)} days</div></td>
                  ${buckets.map(([field]) => {
                    const values = modelKeys.map((key) => rowsForWindow.find((r) => r.model_key === key)?.[field]);
                    return modelKeys.map((key, i) => `<td class="num${performanceWinnerClass(values[i], values)}">${fmtPctValue(values[i])}</td>`).join("");
                  }).join("")}
                </tr>
              `;
            }).join("")}
...
function renderScoreBucketCell(row) {
  if (!row) return `<span class="muted-cell">—</span>`;
  return `
    <div class="bucket-cell">
      <strong>${fmtPctValue(row.hit_rate_pct)}</strong>
      <span>${escapeHtml(row.bucket_label || "Bucket")}</span>
      <small>${fmtNum(row.hits)} / ${fmtNum(row.players)} hits</small>
    </div>
  `;
}

function renderScoreBucketsCard() {
  const orders = Array.from(new Set(appScoreBucketRows.map((row) => Number(row.bucket_order)).filter(Number.isFinite))).sort((a, b) => a - b);
  const modelKeys = ["v1_classic", "v2_proxy", "v3_ml"];

  if (!orders.length) {
    return `
      <section class="board-card performance-card full-width-card">
        <div class="board-header"><div><div class="eyebrow">Score Buckets</div><h2>Hit Rate by Score / Probability Bucket</h2></div></div>
        <div class="empty-state">No score bucket data available.</div>
      </section>
    `;
  }
    ...
    <section class="board-card performance-card full-width-card">
      <div class="board-header">
        <div>
          ...
          <h2>Hit Rate by Score / Probability Bucket</h2>
        </div>
        <div class="board-meta"><span>V1/V2 deciles</span><span>V3 probability buckets</span></div>
      </div>
      <div class="table-wrap performance-table-wrap">
        <table class="performance-table score-bucket-pivot-table">
          <thead>
            <tr>
              <th>Bucket Rank</th>
              ${modelKeys.map((key) => `<th>${modelPill(key)} ${modelDisplayName(key)}</th>`).join("")}
            </tr>
          </thead>
          <tbody>
            ${orders.map((order) => {
              const rows = modelKeys.map((key) => appScoreBucketRows.find((row) => row.model_key === key && Number(row.bucket_order) === Number(order)));
              const values = rows.map((row) => row?.hit_rate_pct);
              return `
                <tr>
                  <td><strong>${order === 1 ? "Highest" : `#${order}`}</strong></td>
                  ${rows.map((row, i) => `<td class="${performanceWinnerClass(values[i], values)}">${renderScoreBucketCell(row)}</td>`).join("")}
                </tr>
              `;
            }).join("")}
          </tbody>
...
function renderFeatureSignalRows() {
  const keys = selectedModelKeys();
  let rows = appFeatureSignalRows
    .filter((row) => keys.includes(row.model_key))
    .sort((a, b) => modelOrder(a.model_key) - modelOrder(b.model_key) || Number(a.feature_order || 99) - Number(b.feature_order || 99));

  if (modelPerformanceView === "all") {
    rows = rows.filter((row) => Number(row.feature_order || 99) <= 5);
  }

  if (!rows.length) {
    return `<tr><td colspan="4" class="empty-state">No feature signal rows available for this model.</td></tr>`;
  }

  const maxByModel = new Map();
  rows.forEach((row) => {
    const key = row.model_key;
    const val = Math.abs(Number(row.signal_value || 0));
    maxByModel.set(key, Math.max(maxByModel.get(key) || 0, val));
  });

  return rows.map((row) => {
    const value = Number(row.signal_value || 0);
    const max = maxByModel.get(row.model_key) || 1;
    const width = Math.max(8, Math.min(100, Math.abs(value) / max * 100));
    return `
      <tr>
        <td>${modelPill(row.model_key)} ${escapeHtml(row.model_name || modelDisplayName(row.model_key))}</td>
        <td><strong>${escapeHtml(row.feature_name || "Feature")}</strong></td>
        <td>
          <div class="signal-bar-track"><div class="signal-bar-fill model-${escapeHtml(row.model_key)}" style="width:${width}%"></div></div>
        </td>
        <td class="num">${value >= 0 ? "+" : ""}${value.toFixed(1)}</td>
      </tr>
    `;
  }).join("");
}
  ...
  return `
    <section class="board-card performance-card">
      <div class="board-header">
        ...
        <div class="board-meta"><span>${modelPerformanceView === "all" ? "Top signals by model" : modelDisplayName(modelPerformanceView)}</span></div>
      </div>
      <div class="table-wrap performance-table-wrap">
        <table class="performance-table feature-signal-table">
          <thead>
            <tr>
              <th>Model</th>
              <th>Feature</th>
          ...
          <tbody>${renderFeatureSignalRows()}</tbody>
        </table>
      </div>
    </section>
  `;
...
function renderPerformancePage() {
  ...
  content.innerHTML = `
    <section class="performance-note">
      ...
      <span>Compare V1, V2, and V3 across bucket accuracy, rolling windows, calibration buckets, and feature signals.</span>
    </section>
    ${renderModelPerformanceControls()}
  ...
  document.querySelectorAll("#modelPerformanceViewButtons .segment").forEach((button) => {
    button.addEventListener("click", () => {
      const next = button.dataset.modelPerformanceView;
      if (!next || next === modelPerformanceView) return;
      modelPerformanceView = next;
      renderPerformancePage();
    });
  });

  document.querySelectorAll("#modelPerformanceRangeButtons .segment").forEach((button) => {
    button.addEventListener("click", () => {
      const next = button.dataset.modelPerformanceRange;
      if (!next || next === modelPerformanceRange) return;
      modelPerformanceRange = next;
      renderPerformancePage();
    });
  });
}
...
function renderHero() {
  ...
  setText("activeWindowLabel", `Recommended · Last ${selectedWindow} Games`);
  ...
  if (topClassic) {
    ...
    setText(
      "heroNarrative",
      `${name} leads the ${redsTeamFilter === "all" ? "Reds game" : redsTeamFilter} recommended board using V2 opportunity-adjusted ranking, with V1 Matchup Score retained as the benchmark.`
    );
    return;
  }
...
async function loadRedsGameBoardData() {
  try {
    ...
    redsClassicRows = withMlbClassicRanks((classicResult.data || [])
      .filter((row) => row.batter_name || row.full_name))
      .sort(sortByRecommendedV2);
...
function redsBoardRows() {
  ...
  return redsBoardMode === "classic"
    ? withMlbClassicRanks(filtered).sort(sortByRecommendedV2)
    : filtered;
}
    ...
    <section class="control-deck performance-window-deck reds-v3-controls">
      <div class="control-group">
        ...
        <div class="segmented" id="redsPredictionModeButtons">
          ...
          <button class="segment ${!isMl ? "active" : ""}" data-reds-mode="classic" type="button">🎯 Recommended</button>
        </div>
      </div>
      ...
      <div class="control-group grow">
        ...
        <div class="sort-pill">Reds vs ${escapeHtml(opponent)} · ${isMl ? "V3 hit probability" : "V2 opportunity-adjusted recommendations with V1 retained"}</div>
      </div>
...
function renderRedsClassicRows(rows) {
  if (!rows.length) {
    return `<tr><td colspan="12" class="empty-state">No recommended hitters match the current filter.</td></tr>`;
  }
  ...
  return rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-reds-classic-player-id="${row.player_id}">
        ...
        <td>
          <div class="player-name">${escapeHtml(row.pitcher_name || "TBD")}</div>
          <div class="player-sub">${escapeHtml(row.pitcher_team_name || row.opponent_team_name || "—")} · ${escapeHtml(row.pitcher_throws || "—")}HP</div>
        </td>
        <td>${row.game_date ? formatGameDate(row.game_date) : "—"}</td>
      </tr>
    `;
...
function renderTable() {
  ...
  setText("redsBoardTitle", isMl ? `Reds vs ${opponent} Hit Probabilities` : `Reds vs ${opponent} Recommended Hitters`);
  setText("redsBoardEyebrow", isMl ? "Game-Specific V3 Board" : "Game-Specific Recommended Board");
  setText("statusPill", `${fmtNum(totalRows)} hitters scored`);
  setText("lastRefresh", isMl ? "ML Prediction" : `Recommended · Last ${selectedWindow}`);
