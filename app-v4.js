...
async function loadModelCompareData() {
  try {
    const { data, error } = await client
      .from("v_matchup_model_experimental_ranked")
      .select("*")
      .eq("selected_window", compareWindow)
      .eq("actual_loaded", true)
      .gt("actual_at_bats", 0)
      .order("game_date", { ascending: false })
      .order("experimental_rank", { ascending: true })
      .limit(500);
    ...
    console.error("Error loading experimental model comparison:", err);
    comparisonRows = [];
    renderModelComparePage();
  }
}
...
function renderHero() {
  ...
  const topMatchup = findMatchup(top.player_id);
  setText(
    "heroScore",
    topMatchup?.matchup_score !== null && topMatchup?.matchup_score !== undefined
  ...
  setText(
    "heroNarrative",
    (() => {
      const matchup = findMatchup(top.player_id);
      if (matchup?.matchup_score !== null && matchup?.matchup_score !== undefined) {
        return `${top.full_name} leads the matchup board with a ${fmtDecimal(matchup.matchup_score, 1)} Matchup Score, ${Number(top.hot_score || 0).toFixed(1)} Hot Score, ${fmtPct(top.hit_rate)} hit rate, and a ${top.current_hit_streak}-game current hit streak.`;
      }
      return `${top.full_name} leads the player-last-${selectedWindow}-games model with a ${Number(top.hot_score || 0).toFixed(1)} Hot Score, ${fmtPct(top.hit_rate)} hit rate, ${top.hits} hits, ${top.extra_base_hits} extra-base hits, and a ${top.current_hit_streak}-game current hit streak.`;
    })()
  );
}
...
function hitRate(rows) {
  ...
  const hits = rows.filter((row) => row.actual_got_hit === true).length;
  return hits / rows.length;
}

function formatRankMove(row) {
...
function compareTopPickStats(rankKey) {
  const rows = comparisonRows.filter((row) => Number(row?.[rankKey] || 0) === 1);
  return {
    count: rows.length,
    hits: rows.filter((row) => row.actual_got_hit === true).length,
    rate: hitRate(rows)
  };
}

function compareTopNStats(rankKey, n) {
  const rows = comparisonRows.filter((row) => Number(row?.[rankKey] || 0) <= n);
  return {
    count: rows.length,
    hits: rows.filter((row) => row.actual_got_hit === true).length,
    rate: hitRate(rows)
...
function renderModelComparePage() {
  ...
  const currentTop = compareTopPickStats("current_rank");
  const experimentalTop = compareTopPickStats("experimental_rank");
  const currentTop5 = compareTopNStats("current_rank", 5);
  const experimentalTop5 = compareTopNStats("experimental_rank", 5);
  const movedRows = comparisonRows.filter((row) => Number(row.current_rank) !== Number(row.experimental_rank));
  const latestDate = comparisonRows[0]?.game_date || null;
  const latestRows = latestDate ? comparisonRows.filter((row) => row.game_date === latestDate).slice(0, 15) : [];

  const averageDelta = comparisonRows.length
    ? comparisonRows.reduce((sum, row) => sum + (Number(row.experimental_matchup_score || 0) - Number(row.current_matchup_score || 0)), 0) / comparisonRows.length
    : null;
  ...
  content.innerHTML = `
    ...
    <section class="hero-grid performance-grid">
      <article class="insight-card compact">
        <div class="metric-icon">🏁</div>
        <div class="metric-label">Current Top Pick</div>
        <div class="metric-value">${fmtRate(currentTop.rate)}</div>
        <div class="metric-sub">${fmtNum(currentTop.hits)} hits / ${fmtNum(currentTop.count)} picks</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">🧪</div>
        <div class="metric-label">Experimental Top Pick</div>
        <div class="metric-value">${fmtRate(experimentalTop.rate)}</div>
        <div class="metric-sub">${fmtNum(experimentalTop.hits)} hits / ${fmtNum(experimentalTop.count)} picks</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">⑤</div>
        <div class="metric-label">Top 5 Current</div>
        <div class="metric-value">${fmtRate(currentTop5.rate)}</div>
        <div class="metric-sub">${fmtNum(currentTop5.hits)} hits / ${fmtNum(currentTop5.count)} picks</div>
      </article>

      <article class="insight-card compact">
        <div class="metric-icon">⑤</div>
        <div class="metric-label">Top 5 Experimental</div>
        <div class="metric-value">${fmtRate(experimentalTop5.rate)}</div>
        <div class="metric-sub">${fmtNum(experimentalTop5.hits)} hits / ${fmtNum(experimentalTop5.count)} picks</div>
      </article>
    </section>

    <section class="performance-note">
      <strong>Shadow model comparison.</strong>
      <span>The experimental model reduces pitcher weighting and increases hitter form + batter split weighting. It does not replace the live Hot Board yet.</span>
    </section>
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        <div>
          ...
          <h2>Current vs Experimental Model</h2>
        </div>
        <div class="board-meta">
          <span>${fmtNum(comparisonRows.length)} AB opportunities</span>
          <span>${fmtNum(movedRows.length)} changed ranks</span>
          <span>Avg score delta ${averageDelta === null ? "—" : averageDelta.toFixed(1)}</span>
        </div>
      </div>
        ...
        <table class="performance-table">
          <thead>
            <tr>
              ...
              <th class="num">Exp Rank</th>
              <th class="num">Move</th>
              <th class="num">Current Score</th>
              <th class="num">Exp Score</th>
              <th class="num">AB</th>
              <th class="num">H</th>
              <th>Result</th>
            </tr>
          ...
          <tbody>
            ${comparisonRows.length ? comparisonRows.slice(0, 120).map((row) => `
              <tr>
                <td>${formatGameDate(row.game_date)}</td>
                <td>${row.full_name || "—"}</td>
                <td>${row.opponent_team_name || "—"}</td>
                <td class="num">${fmtNum(row.current_rank)}</td>
                <td class="num">${fmtNum(row.experimental_rank)}</td>
                <td class="num">${formatRankMove(row)}</td>
                <td class="num">${fmtDecimal(row.current_matchup_score, 1)}</td>
                <td class="num">${fmtDecimal(row.experimental_matchup_score, 1)}</td>
                <td class="num">${fmtNum(row.actual_at_bats)}</td>
                <td class="num">${fmtNum(row.actual_hits)}</td>
                <td>${row.actual_got_hit ? "✅ Hit" : "❌ No hit"}</td>
              </tr>
            `).join("") : `<tr><td colspan="11" class="empty-state">No scored predictions loaded yet for this window.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        <div>
          <div class="eyebrow">Latest Game</div>
          <h2>Score Changes for ${formatGameDate(latestDate)}</h2>
        </div>
        <div class="board-meta">
          <span>Sorted by experimental rank</span>
        </div>
      </div>
        ...
        <table class="performance-table">
          ...
          <tbody>
            ${latestRows.length ? latestRows.map((row) => {
              const scoreChange = Number(row.experimental_matchup_score || 0) - Number(row.current_matchup_score || 0);
              ...
              return `
                <tr>
                  ...
                  <td class="num">${fmtDecimal(row.current_matchup_score, 1)}</td>
                  <td class="num">${fmtDecimal(row.experimental_matchup_score, 1)}</td>
                  <td class="num">${sign}${scoreChange.toFixed(1)}</td>
                  <td class="num">${fmtNum(row.current_rank)}</td>
                  <td class="num">${fmtNum(row.experimental_rank)}</td>
                  <td>${row.actual_got_hit ? "✅ Hit" : "❌ No hit"}</td>
                </tr>
              `;
            }).join("") : `<tr><td colspan="7" class="empty-state">No latest game comparison available.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function showView(viewName) {
  ...
  if (viewName === "compare") {
    setText("pageTitle", "Experimental Model Compare");
    setText("pageSubtitle", `Compare current matchup scores against the reduced-pitcher-weight experimental model for the ${compareWindow}-game window.`);
