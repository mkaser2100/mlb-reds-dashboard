...
console.info("MLB Hit Lab app-v4 loaded: v3-driver-payload-preservation-v3");
...
function openV3MlbDrawer(row) {
  ...
  const positiveDrivers = Array.isArray(row.drawer_top_positive_drivers)
    ? row.drawer_top_positive_drivers
    : (Array.isArray(row.top_positive_drivers) ? row.top_positive_drivers : []);
  const riskFactors = Array.isArray(row.drawer_top_risk_factors)
    ? row.drawer_top_risk_factors
    : (Array.isArray(row.top_risk_factors) ? row.top_risk_factors : []);
...
function normalizeV3HitRow(row) {
  ...
  return {
    ...
    // Preserve exact contribution payloads after normalization. These arrays are
    // attached before normalizeV3HitRow() and were previously dropped here,
    // forcing the drawer into its legacy tag/narrative fallback.
    ...
    drawer_top_risk_factors: Array.isArray(row.drawer_top_risk_factors)
      ? row.drawer_top_risk_factors
      : (Array.isArray(row.top_risk_factors) ? row.top_risk_factors : []),
    drawer_feature_count: Number(row.drawer_feature_count ?? row.feature_count ?? 0),
