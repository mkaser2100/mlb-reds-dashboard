...
console.info("MLB Hit Lab app-v4 loaded: v3-driver-sync-merge-v4");
...
function openV3MlbDrawer(row) {
  // Re-attach the already-loaded explanation payload at click time. This is
  // synchronous, so the drawer opens immediately and never waits on a second
  // Supabase request. It also protects against later row-normalization or V2
  // enrichment steps accidentally dropping the driver arrays.
  const matchedExplanation = (v3DrawerExplanationRows || []).find((explanation) =>
    (explanation?.prediction_id != null && row?.prediction_id != null &&
      String(explanation.prediction_id) === String(row.prediction_id)) ||
    (explanation?.player_id != null && row?.player_id != null &&
      String(explanation.player_id) === String(row.player_id) &&
      String(explanation.prediction_run_date || "") === String(row.prediction_run_date || ""))
  );

  if (matchedExplanation) {
    row = mergeV3DrawerExplanations([row], [matchedExplanation])[0] || row;
  }
