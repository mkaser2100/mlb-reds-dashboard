...
console.info("MLB Hit Lab app-v4 loaded: market-edge-cache-ui-v1");
...
function mpFeatureRows() {
  ...
  // Backward-compatible fallback for the old V1-only one-row component payload.
  const source = rows?.[0];
  if (!source) return [];
  const components = [
...
function mpFeatureSubtext(row) {
  ...
  if (row.model === "V3") {
    return "Positive signal from evaluated V3 feature separation";
  }
