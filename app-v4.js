...
console.info("MLB Hit Lab app-v4 loaded: v3-diagnostics-v8");
...
async function loadV3DrawerExplanationForRow(row) {
  ...
  const runQuery = async (includeGame) => {
    let query = client
      .from("v_mlb_v3_player_drawer_explanations")
      .select("*")
      .eq("prediction_run_date", predictionDate)
    ...
    if (includeGame && gamePk !== null && gamePk !== undefined && gamePk !== "") {
      query = query.eq("game_pk", gamePk);
    }

    return query.order("rank_overall", { ascending: true }).limit(20);
  };

  try {
    let { data, error } = await runQuery(true);
    let lookupMode = "player + date + game";

    if (error || !data?.length) {
      ({ data, error } = await runQuery(false));
      lookupMode = "player + date fallback";
    }
    ...
    const snapshot = v3DiagnosticSnapshot(
      row,
      candidates,
      explanation,
      `${lookupMode}; matched by ${matchMethod}`
    );
    ...
    console.error("Error loading direct V3 drawer explanation:", snapshot);
    return null;
  }
}
...
async function openV3MlbDrawer(row) {
  ...
  if (!hasExactContributions && !row.__drawerDirectLookupAttempted) {
    ...
    if (directExplanation) {
      drawerDiagnostics = directExplanation.__v3DiagnosticSnapshot || drawerDiagnostics;
      ...
      ${positiveDrivers.length
        ...
        : `
          <div class="v3-driver-unavailable" role="status">
            ...
            <span>The prediction loaded, but its feature-contribution record did not match this player and game. Refresh after the V3 explanation pipeline completes.</span>
          </div>
        `}
