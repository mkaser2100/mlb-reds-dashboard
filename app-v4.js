...
console.info("MLB Hit Lab app-v4 loaded: todays-outlook-consensus-v2");
...
function getMlbConsensusPlays(limit = 3) {
  return (mlbRows || [])
    .filter((row) => Number.isFinite(Number(row?.consensus_v2_rank)) && Number.isFinite(Number(row?.consensus_v3_rank)))
    .sort((a, b) => {
  ...
  return `
    <section class="daily-summary-card consensus-outlook-card">
      <div class="outlook-main">
        ...
        ` : `
          <p>
            ${topPick ? `${escapeHtml(topPick.batter_name || topPick.full_name)} is the top ML play at ${fmtProbabilityPct(topPick)}. Waiting for V2/V3 overlap before showing consensus plays.` : "Waiting for today's V3 predictions."}
          </p>
        `}
      </div>
