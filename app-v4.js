...
function openMlbDrawer(playerId) {
  ...
  setText(
    "drawerPlayerSub",
    `${row.team_name || "MLB"} · Player ID ${playerId} · ${mlbBoardMode === "classic" ? "classic matchup score" : "ML hit probability model"}`
  );
...
function renderMlbHitBoardPage(error = null) {
  ...
  content.innerHTML = `
    ${renderMlbPredictionModeControls("ml")}    ${renderModelStatusPanel()}
...
function wireEvents() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (modeButton) {
      ...
      if (nextMode === "classic" && !mlbClassicRows.length) {
