...
console.info("MLB Hit Lab app-v4 loaded: v3-composite-row-key-v7");
...
async function loadV3DrawerExplanationForRow(row) {
  ...
  if (!predictionDate || playerId === null || playerId === undefined) return null;
  ...
  const runQuery = async (includeGame) => {
    ...
    return query.order("rank_overall", { ascending: true }).limit(10);
  };
  ...
  try {
    ...
    const explanation = exactGame || candidates[0] || null;

    console.info("V3 drawer direct lookup result", {
      predictionDate,
      playerId,
      gamePk,
      candidates: candidates.length,
      matchedGamePk: explanation?.game_pk ?? null,
      positiveDrivers: v3JsonArray(explanation?.top_positive_drivers).length,
      riskFactors: v3JsonArray(explanation?.top_risk_factors).length,
      groups: v3JsonArray(explanation?.group_contributions).length
    });
...
function openMlbDrawer(playerId, gamePk = null) {
  ...
  const exactRow = normalizedGamePk
    ? activeMlbRows.find(
        (x) =>
          String(x.player_id ?? "") === normalizedPlayerId &&
          String(x.game_pk ?? "") === normalizedGamePk
  ...
  if (!baseRow) {
    console.error("Unable to resolve MLB drawer row", {
      playerId,
      gamePk,
      boardMode: mlbBoardMode,
      availableMatches: activeMlbRows
        .filter((x) => String(x.player_id ?? "") === normalizedPlayerId)
        .map((x) => ({
          player_id: x.player_id,
          game_pk: x.game_pk,
          prediction_id: x.prediction_id,
          prediction_run_date: x.prediction_run_date
        }))
    });
    return;
  }
