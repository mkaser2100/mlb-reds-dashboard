...
console.info("MLB Hit Lab app-v4 loaded: performance-v3-datafix-v1");
...
const V2_ENHANCEMENT_SELECT = "player_id,full_name,team_id,team_name,game_pk,matchup_score,model_v2_score,model_confidence,expected_plate_appearances,recent_lineup_spot,adjusted_batter_split_avg,adjusted_pitcher_baa_split,adjusted_batter_split_score,adjusted_pitcher_vulnerability_score";
let searchTerm = "";
...
function mergeV2Enhancements(rows, v2Rows) {
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      v1_matchup_score: v2.matchup_score,
...
function rowPlayerName(row) {
  return row?.batter_name || row?.full_name || "—";
}

function v3RankValue(row) {
  const rank = Number(row?.rank_overall);
  return Number.isFinite(rank) && rank > 0 ? rank : 9999;
}

function v2RankValue(row) {
  const rank = Number(row?.v2_consensus_rank || row?.v2_rank || row?.recommended_rank);
  return Number.isFinite(rank) && rank > 0 ? rank : 9999;
}

function addV2ConsensusRanks(rows) {
  const byKey = new Map();

  rows
    .slice()
    .filter((row) => Number.isFinite(Number(row?.model_v2_score)))
    .sort((a, b) => {
      const v2Delta = Number(b.model_v2_score || 0) - Number(a.model_v2_score || 0);
      if (v2Delta !== 0) return v2Delta;

      const v3Delta = v3RankValue(a) - v3RankValue(b);
      if (v3Delta !== 0) return v3Delta;

      return String(rowPlayerName(a)).localeCompare(String(rowPlayerName(b)));
    })
    .forEach((row, index) => {
      const key = v2LookupKey(row) || String(row?.player_id || rowPlayerName(row));
  ...
  return rows.map((row) => {
    ...
    return {
      ...row,
      v2_consensus_rank: byKey.get(key) || null
    };
  });
}

function outlookReasonCandidates(row) {
  const reasons = [];
  const v2Rank = v2RankValue(row);
  const v3Rank = v3RankValue(row);
  const expectedPa = Number(row?.expected_plate_appearances || 0);
  const v1Score = Number(row?.v1_matchup_score || 0);
  const hitPct = Number(row?.hit_probability_pct || 0);

  if (v2Rank <= 5 && v3Rank <= 5) reasons.push("elite model agreement");
  else if (v2Rank <= 10 && v3Rank <= 10) reasons.push("strong model agreement");

  if (hitPct >= 70) reasons.push("high ML confidence");
  if (v1Score >= 75) reasons.push("elite matchup");
  if (expectedPa >= 4.5) reasons.push("top-of-lineup volume");
  else if (expectedPa >= 4.2) reasons.push("strong PA outlook");

  if (Number(row?.adjusted_batter_split_score || 0) >= 70) reasons.push("strong split signal");
  if (Number(row?.adjusted_pitcher_vulnerability_score || 0) >= 70) reasons.push("favorable pitcher profile");

  return [...new Set(reasons)].slice(0, 2);
}

function consensusLevel(row) {
  const v2Rank = v2RankValue(row);
  const v3Rank = v3RankValue(row);

  if (v2Rank <= 5 && v3Rank <= 5) return { icon: "🔥", label: "Elite" };
  if (v2Rank <= 10 && v3Rank <= 10) return { icon: "🎯", label: "Strong" };
  return { icon: "✓", label: "Aligned" };
}

function topConsensusOutlookRows(rows) {
  const eligible = (rows || [])
    .filter((row) => row?.player_id && Number.isFinite(Number(row?.model_v2_score)) && v3RankValue(row) < 9999)
    .map((row) => ({
      ...row,
      consensus_sort: v2RankValue(row) + v3RankValue(row),
      consensus_max_rank: Math.max(v2RankValue(row), v3RankValue(row))
    }))
    .filter((row) => v2RankValue(row) <= 25 && v3RankValue(row) <= 25)
    .sort((a, b) => {
      const maxDelta = a.consensus_max_rank - b.consensus_max_rank;
      if (maxDelta !== 0) return maxDelta;

      const sumDelta = a.consensus_sort - b.consensus_sort;
      if (sumDelta !== 0) return sumDelta;

      return Number(b.hit_probability_pct || 0) - Number(a.hit_probability_pct || 0);
    });

  return eligible.slice(0, 3);
}

function renderTodayOutlookConsensus(rows, topPick) {
  const consensusRows = topConsensusOutlookRows(rows);

  if (!consensusRows.length) {
    return `
      <p>
        ${topPick ? `${rowPlayerName(topPick)} is the top ML play at ${fmtProbabilityPct(topPick)}. Recommended consensus will appear once V2 and V3 both score today's board.` : "Waiting for today's V3 predictions."}
      </p>
    `;
  }

  return `
    <div class="outlook-consensus-list" aria-label="Top V2 and V3 consensus plays">
      ${consensusRows.map((row, index) => {
        const level = consensusLevel(row);
        const reasons = outlookReasonCandidates(row);
        return `
          <button class="outlook-consensus-row" type="button" data-consensus-player-id="${row.player_id}" title="Jump to ${escapeHtml(rowPlayerName(row))} on the board">
            <span class="outlook-medal">${index === 0 ? level.icon : index === 1 ? "②" : "③"}</span>
            <span class="outlook-player-block">
              <strong>${escapeHtml(rowPlayerName(row))}</strong>
              <small>${reasons.length ? reasons.join(" • ") : `${level.label} consensus play`}</small>
            </span>
            <span class="outlook-rank-pills">
              <span class="model-rank-pill v2">V2 #${fmtNum(v2RankValue(row))}</span>
              <span class="model-rank-pill v3">V3 #${fmtNum(v3RankValue(row))}</span>
            </span>
          </button>
        `;
      }).join("")}
...
function scrollToMlbPlayer(playerId) {
  const safePlayerId = window.CSS?.escape ? CSS.escape(String(playerId)) : String(playerId).replace(/"/g, '\\"');
  const row = document.querySelector(`.mlb-clickable-row[data-mlb-player-id="${safePlayerId}"]`);
  if (!row) return false;

  row.scrollIntoView({ behavior: "smooth", block: "center" });
  row.classList.add("consensus-row-highlight");
  window.setTimeout(() => row.classList.remove("consensus-row-highlight"), 2200);
  return true;
}
...
async function loadMlbHitBoardData() {
  try {
    ...
    mlbRows = addV2ConsensusRanks(mergeV2Enhancements(
      (v3Result.data || [])
        .filter((row) => row.batter_name || row.full_name)
        .map(normalizeV3HitRow),
      mlbV2EnhancementRows
...
function renderMlbHitBoardPage(error = null) {
  ...
  content.innerHTML = `
    ...
    <section class="daily-summary-card outlook-card">
      <div class="outlook-content">
        <div class="eyebrow">Daily Summary</div>
        <h2>Today's Outlook</h2>
        ${renderTodayOutlookConsensus(rows, topPick)}
      </div>
      <div class="summary-metrics outlook-metrics">
        <span>${fmtNum(rows.length)} hitters scored</span>
        <span>${top25.length} shown</span>
        <span>${topConsensusOutlookRows(rows).length || 0} consensus</span>
      </div>
    </section>
...
function wireEvents() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (consensusButton) {
      ...
      const didScroll = scrollToMlbPlayer(consensusButton.dataset.consensusPlayerId);
      if (!didScroll) openMlbDrawer(consensusButton.dataset.consensusPlayerId);
      return;
    }
...
/* --------------------------------------------------------------------------
   Model Performance v3 redesign
   Global model selector + 4-card comparison page.
   -------------------------------------------------------------------------- */
let performanceModelFilter = "all";
let performanceScorecardRange = "season";
let performanceDailyScorecardRows = [];
let performanceRollingBucketRows = [];
let performanceCalibrationRows = [];
let performanceV3CalibrationRows = [];
let performanceComponentRows = [];
let performanceV2DiagnosticRows = [];
let performanceV3FeatureRows = [];

const PERFORMANCE_MODEL_DEFS = {
  v1_classic: { key: "v1_classic", short: "V1", name: "V1 Classic", className: "model-v1" },
  v2_proxy: { key: "v2_proxy", short: "V2", name: "V2 Recommended", className: "model-v2" },
  v3_ml: { key: "v3_ml", short: "V3", name: "V3 ML", className: "model-v3" }
};

function normalizePerformanceModelKey(value) {
  const x = String(value || "").toLowerCase();
  if (x === "v1" || x === "v1_classic") return "v1_classic";
  if (x === "v2" || x === "v2_proxy" || x === "v2_recommended") return "v2_proxy";
  if (x === "v3" || x === "v3_ml") return "v3_ml";
  return x || "unknown";
}

function performanceModelDef(key) {
  return PERFORMANCE_MODEL_DEFS[normalizePerformanceModelKey(key)] || {
    key: normalizePerformanceModelKey(key),
    short: String(key || "?").toUpperCase(),
    name: String(key || "Unknown Model"),
    className: "model-neutral"
  };
}

function performanceVisibleModels() {
  if (performanceModelFilter === "all") return ["v1_classic", "v2_proxy", "v3_ml"];
  return [performanceModelFilter];
}

function pctFromRatioOrPct(value, digits = 0) {
  if (value === null || value === undefined || Number.isNaN(Number(value))) return "—";
  const n = Number(value);
...
function pctValueFromRatioOrPct(value) {
  if (value === null || value === undefined || Number.isNaN(Number(value))) return null;
  const n = Number(value);
  return Math.abs(n) <= 1 ? n * 100 : n;
}

function safeDividePct(numerator, denominator) {
...
function modelBadgeHtml(modelKey, labelOverride = null) {
  const def = performanceModelDef(modelKey);
  return `<span class="model-badge ${def.className}">${escapeHtml(labelOverride || def.short)}</span>`;
}

function renderPerformanceGlobalSelector() {
  const options = [
    { key: "all", label: "All" },
    { key: "v1_classic", label: "V1" },
    { key: "v2_proxy", label: "V2" },
    { key: "v3_ml", label: "V3" }
  ];

  return `
    <section class="control-deck performance-global-deck">
      <div class="control-group">
        <div class="control-label">Model View</div>
        <div class="segmented model-toggle" id="performanceModelButtons">
          ${options.map((option) => `
            <button class="segment ${performanceModelFilter === option.key ? "active" : ""}" data-performance-model="${option.key}" type="button">
              ${option.label}
            </button>
          `).join("")}
        </div>
      ...
      <div class="control-group grow">
        <div class="control-label">Page Focus</div>
        <div class="sort-pill">Comparing V1 Classic, V2 opportunity-adjusted, and V3 ML performance</div>
      </div>
    </section>
  `;
}

async function safePerformanceQuery(label, queryPromise, fallbackValue = []) {
  try {
    const result = await queryPromise;
    if (result?.error) {
      console.error(`Model Performance query failed: ${label}`, result.error);
      return fallbackValue;
    }
    return result?.data ?? fallbackValue;
  } catch (err) {
    console.error(`Model Performance query exception: ${label}`, err);
    return fallbackValue;
  }
}
...
async function loadPerformanceData() {
  try {
    const [
      ...
      safePerformanceQuery(
        "v_mlb_daily_model_scorecard",
        client
          .from("v_mlb_daily_model_scorecard")
          .select("*")
          .order("prediction_run_date", { ascending: true }),
        []
      ),

      safePerformanceQuery(
        "v_mlb_model_rolling_bucket_performance",
        client
          .from("v_mlb_model_rolling_bucket_performance")
          .select("*")
      ...
      safePerformanceQuery(
        ...
        client
          .from("v_mlb_model_phase1_v1_v2_calibration")
          .select("*")
          .eq("selected_window", performanceWindow)
          .order("model_version", { ascending: true })
          .order("score_decile", { ascending: true }),
        []
      ),

      safePerformanceQuery(
        "v_mlb_v3_calibration_daily",
        client
          .from("v_mlb_v3_calibration_daily")
          .select("*")
          .order("prediction_run_date", { ascending: true })
      ...
      safePerformanceQuery(
        "v_mlb_model_component_backtest",
        client
          .from("v_mlb_model_component_backtest")
          .select("*")
          .order("selected_window", { ascending: true }),
        []
      ),
      ...
      safePerformanceQuery(
        "v_mlb_v2_feature_diagnostics_summary",
        client
          .from("v_mlb_v2_feature_diagnostics_summary")
          .select("*"),
        []
      ),

      safePerformanceQuery(
        "v_mlb_ml_training_features_v3_hit_1plus",
        client
          .from("v_mlb_ml_training_features_v3_hit_1plus")
          .select("target_hit_1plus,matchup_score,recent_form_score,batter_split_score,pitcher_vulnerability_score,pitcher_recent_form_score,batter_recent_hit_rate,pitcher_last5_whip")
          .limit(10000),
        []
      ),

      safePerformanceQuery(
        "v_mlb_hit_board_performance_summary",
        client
          .from("v_mlb_hit_board_performance_summary")
          .select("*")
          .eq("selected_window", performanceWindow)
      ...
      safePerformanceQuery(
        "v_mlb_hit_board_rank_performance",
        client
          .from("v_mlb_hit_board_rank_performance")
          .select("*")
          .eq("selected_window", performanceWindow)
      ...
      safePerformanceQuery(
        "v_mlb_hit_board_component_analysis",
        client
          .from("v_mlb_hit_board_component_analysis")
          .select("*")
          .eq("selected_window", performanceWindow)
    ...
    performanceDailyScorecardRows = dailyScorecardData || [];
    performanceRollingBucketRows = rollingBucketData || [];
    performanceCalibrationRows = calibrationData || [];
    performanceV3CalibrationRows = v3CalibrationData || [];
    performanceComponentRows = componentData || [];
    performanceV2DiagnosticRows = v2DiagnosticsData || [];
    performanceV3FeatureRows = v3FeaturesData || [];

    // Keep legacy variables populated as fallback / drawer compatibility.
    mlbPerformanceSummary = legacySummaryData || null;
    mlbRankPerformanceRows = legacyRankData || [];
    mlbScoreBucketRows = legacyBucketData || [];
    mlbWindowComparisonRows = legacyWindowData || [];
    mlbComponentAnalysis = legacyComponentData || null;

    console.info("Model Performance rows loaded", {
      scorecard: performanceDailyScorecardRows.length,
      rolling: performanceRollingBucketRows.length,
      calibration: performanceCalibrationRows.length,
      v3Calibration: performanceV3CalibrationRows.length,
      v2Diagnostics: performanceV2DiagnosticRows.length,
      v3Features: performanceV3FeatureRows.length
    });

    renderPerformancePage();
  } catch (err) {
    console.error("Error loading model performance data:", err);
    performanceDailyScorecardRows = [];
    performanceRollingBucketRows = [];
    performanceCalibrationRows = [];
    performanceV3CalibrationRows = [];
    performanceComponentRows = [];
    performanceV2DiagnosticRows = [];
    performanceV3FeatureRows = [];
    renderPerformancePage();
  }
}

function scorecardRowsForRange(rangeKey) {
  const rows = (performanceDailyScorecardRows || []).filter((row) => Number(row.played_rows || 0) > 0);
  if (!rows.length) return [];

  if (rangeKey === "season") return rows;

  const dates = rows
    .map((row) => row.prediction_run_date)
    .filter(Boolean)
    .sort();
  const maxDateText = dates[dates.length - 1];
  if (!maxDateText) return rows;

  const maxDate = new Date(`${maxDateText}T12:00:00`);
  const days = rangeKey === "last7" ? 7 : 30;
  const minDate = new Date(maxDate);
  minDate.setDate(maxDate.getDate() - days + 1);

  return rows.filter((row) => {
    const date = new Date(`${row.prediction_run_date}T12:00:00`);
    return date >= minDate && date <= maxDate;
  });
}

function aggregateScorecardRows(rangeKey) {
  const rows = scorecardRowsForRange(rangeKey);
  const byModel = new Map();

  rows.forEach((row) => {
    const key = normalizePerformanceModelKey(row.model_key);
    if (!PERFORMANCE_MODEL_DEFS[key]) return;
    if (!byModel.has(key)) {
      byModel.set(key, {
        model_key: key,
        model_name: performanceModelDef(key).name,
        dates: new Set(),
        top1_hits: 0, top1_played: 0,
        top5_hits: 0, top5_played: 0,
        top10_hits: 0, top10_played: 0,
        top20_hits: 0, top20_played: 0,
        top25_hits: 0, top25_played: 0,
        overall_hits: 0, played_rows: 0
      });
    }

    const acc = byModel.get(key);
    if (row.prediction_run_date) acc.dates.add(row.prediction_run_date);
    [1, 5, 10, 20, 25].forEach((bucket) => {
      acc[`top${bucket}_hits`] += Number(row[`top${bucket}_hits`] || 0);
      acc[`top${bucket}_played`] += Number(row[`top${bucket}_played`] || 0);
    });
    acc.overall_hits += Number(row.overall_hits || 0);
    acc.played_rows += Number(row.played_rows || 0);
  });

  return Array.from(byModel.values()).map((row) => ({
    ...row,
    days: row.dates.size,
    top1: safeDividePct(row.top1_hits, row.top1_played),
    top5: safeDividePct(row.top5_hits, row.top5_played),
    top10: safeDividePct(row.top10_hits, row.top10_played),
    top20: safeDividePct(row.top20_hits, row.top20_played),
    top25: safeDividePct(row.top25_hits, row.top25_played),
    overall: safeDividePct(row.overall_hits, row.played_rows)
  }));
}

function renderScorecardRangeButtons() {
  const options = [
  ...
  return `
    <div class="segmented compact-segmented" id="performanceScorecardRangeButtons">
      ${options.map((option) => `
        <button class="segment ${performanceScorecardRange === option.key ? "active" : ""}" data-scorecard-range="${option.key}" type="button">${option.label}</button>
      `).join("")}
    </div>
  `;
}
...
function winnerClass(value, maxValue) {
  if (value === null || value === undefined || maxValue === null || maxValue === undefined) return "";
  return Math.abs(Number(value) - Number(maxValue)) < 0.0001 ? " perf-winner" : "";
}

function renderModelScorecardCard() {
  const rows = aggregateScorecardRows(performanceScorecardRange)
    .filter((row) => performanceVisibleModels().includes(row.model_key));

  if (!rows.length) {
    return `
      <section class="board-card performance-card performance-scorecard-card">
        <div class="board-header"><div><div class="eyebrow">Model Scorecard</div><h2>Cross-Model Bucket Performance</h2></div></div>
        <div class="empty-state">No model scorecard rows available yet.</div>
      </section>
    `;
  }

  const buckets = ["top1", "top5", "top10", "top20", "top25", "overall"];
  const maxByBucket = Object.fromEntries(buckets.map((bucket) => [bucket, Math.max(...rows.map((row) => Number(row[bucket] ?? -Infinity)))]));
  ...
  return `
    <section class="board-card performance-card performance-scorecard-card">
      <div class="board-header">
        ...
        <div class="board-meta performance-card-actions">
          ${renderScorecardRangeButtons()}
          <span>${rows.reduce((max, row) => Math.max(max, row.days || 0), 0)} days</span>
        </div>
      </div>
      <div class="table-wrap">
        <table class="performance-table model-scorecard-table">
          <thead>
            <tr>
              <th>Model</th>
              <th class="num">Top Pick</th>
          ...
          <tbody>
            ${rows.map((row) => {
              const def = performanceModelDef(row.model_key);
              return `
                <tr>
                  <td>${modelBadgeHtml(row.model_key)} <strong>${escapeHtml(def.name)}</strong></td>
                  ${buckets.map((bucket) => `
                    <td class="num${winnerClass(row[bucket], maxByBucket[bucket])}">${row[bucket] === null || row[bucket] === undefined ? "—" : `${Math.round(row[bucket])}%`}</td>
                  `).join("")}
                </tr>
              `;
            }).join("")}
...
function aggregateRollingRows() {
  const grouped = new Map();
  (performanceRollingBucketRows || []).forEach((row) => {
    const modelKey = normalizePerformanceModelKey(row.model_key);
    if (!PERFORMANCE_MODEL_DEFS[modelKey]) return;
    if (!performanceVisibleModels().includes(modelKey)) return;

    const key = `${row.window_days}|${modelKey}`;
    if (!grouped.has(key)) {
      grouped.set(key, {
        window_days: Number(row.window_days || 0),
        window_label: row.window_label || `Last ${row.window_days}`,
        model_key: modelKey,
        days_in_window: row.days_in_window,
        top1: null, top5: null, top10: null, top20: null, top25: null
      });
    }
    const bucket = Number(row.bucket_size);
    if ([1, 5, 10, 20, 25].includes(bucket)) grouped.get(key)[`top${bucket}`] = Number(row.hit_rate_pct);
  });

  return Array.from(grouped.values()).sort((a, b) => {
    const windowDelta = a.window_days - b.window_days;
    if (windowDelta) return windowDelta;
    return a.model_key.localeCompare(b.model_key);
  });
}

function renderRollingPerformanceCard() {
  const rows = aggregateRollingRows();
  return `
    <section class="board-card performance-card">
      <div class="board-header">
        <div>
          <div class="eyebrow">Rolling Performance</div>
          <h2>Recent Bucket Hit Rates</h2>
        </div>
        <div class="board-meta">
          <span>Last 7 / 14 / 30 / Season</span>
          <span>No-AB players excluded</span>
        </div>
      </div>
      <div class="table-wrap">
        <table class="performance-table compact-performance-table">
          <thead>
            <tr>
              <th>Window</th>
              ${performanceModelFilter === "all" ? "<th>Model</th>" : ""}
              <th class="num">Top Pick</th>
              <th class="num">Top 5</th>
              <th class="num">Top 10</th>
              <th class="num">Top 20</th>
          ...
          <tbody>
            ${rows.length ? rows.map((row) => `
              <tr>
                <td><strong>${escapeHtml(row.window_label || "—")}</strong></td>
                ${performanceModelFilter === "all" ? `<td>${modelBadgeHtml(row.model_key)} ${escapeHtml(performanceModelDef(row.model_key).short)}</td>` : ""}
                <td class="num">${pctFromRatioOrPct(row.top1)}</td>
                <td class="num">${pctFromRatioOrPct(row.top5)}</td>
                <td class="num">${pctFromRatioOrPct(row.top10)}</td>
                <td class="num">${pctFromRatioOrPct(row.top20)}</td>
                <td class="num">${pctFromRatioOrPct(row.top25)}</td>
              </tr>
            `).join("") : `<tr><td colspan="${performanceModelFilter === "all" ? 7 : 6}" class="empty-state">No rolling model performance rows available.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function modelVersionToKey(modelVersion) {
  const x = String(modelVersion || "").toUpperCase();
  if (x === "V1") return "v1_classic";
  if (x.includes("V2")) return "v2_proxy";
  return normalizePerformanceModelKey(modelVersion);
}

function v3AggregatedCalibrationBuckets() {
  const byBucket = new Map();
  (performanceV3CalibrationRows || []).forEach((row) => {
    const key = row.probability_bucket || "Unknown";
    if (!byBucket.has(key)) {
      byBucket.set(key, {
        model_key: "v3_ml",
        bucket_label: key,
        bucket_order: Number(row.bucket_order || 999),
        predictions: 0,
        hits: 0
      });
    }
  ...
  return Array.from(byBucket.values()).map((row) => ({
    ...row,
    hit_rate: safeDividePct(row.hits, row.predictions)
  })).sort((a, b) => a.bucket_order - b.bucket_order);
}

function calibrationRowsForModel(modelKey) {
  if (modelKey === "v3_ml") return v3AggregatedCalibrationBuckets();
  return (performanceCalibrationRows || [])
    .filter((row) => modelVersionToKey(row.model_version) === modelKey)
    .map((row) => ({
      model_key: modelKey,
      bucket_label: `D${row.score_decile}: ${fmtDecimal(row.score_min, 1)}-${fmtDecimal(row.score_max, 1)}`,
      bucket_order: Number(row.score_decile || 999),
      predictions: Number(row.observations || 0),
      hits: Math.round(Number(row.observations || 0) * Number(row.actual_hit_rate || 0)),
      hit_rate: pctValueFromRatioOrPct(row.actual_hit_rate)
    }))
    .sort((a, b) => a.bucket_order - b.bucket_order);
}

function renderScoreBucketsCard() {
  const visible = performanceVisibleModels();
  const rows = visible.flatMap((modelKey) => calibrationRowsForModel(modelKey).slice(0, performanceModelFilter === "all" ? 5 : 10));
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        <div>
          ...
          <h2>Hit Rate by Model Score</h2>
        </div>
        <div class="board-meta">
          <span>${performanceModelFilter === "all" ? "Top buckets by model" : performanceModelDef(performanceModelFilter).name}</span>
        ...
        <table class="performance-table compact-performance-table">
          ...
          <tbody>
            ${rows.length ? rows.map((row) => `
              <tr>
                ${performanceModelFilter === "all" ? `<td>${modelBadgeHtml(row.model_key)} ${escapeHtml(performanceModelDef(row.model_key).short)}</td>` : ""}
                <td><strong>${escapeHtml(row.bucket_label)}</strong></td>
                <td class="num">${fmtNum(row.predictions)}</td>
                <td class="num">${fmtNum(row.hits)}</td>
                <td class="num">${row.hit_rate === null || row.hit_rate === undefined ? "—" : `${Math.round(row.hit_rate)}%`}</td>
              </tr>
            `).join("") : `<tr><td colspan="${performanceModelFilter === "all" ? 5 : 4}" class="empty-state">No score bucket data available.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function v1FeatureRows() {
  const row = (performanceComponentRows || []).find((item) => Number(item.selected_window) === Number(performanceWindow)) || mlbComponentAnalysis;
  if (!row) return [];
  return [
    { feature: "Recent Form", value: Number(row.recent_form_delta || 0), detail: fmtSignedDelta(row.avg_recent_form_when_hit, row.avg_recent_form_when_no_hit) },
    { feature: "Batter Split", value: Number(row.batter_split_delta || 0), detail: fmtSignedDelta(row.avg_batter_split_when_hit, row.avg_batter_split_when_no_hit) },
    { feature: "SP Vulnerability", value: Number(row.pitcher_vuln_delta || 0), detail: fmtSignedDelta(row.avg_pitcher_vuln_when_hit, row.avg_pitcher_vuln_when_no_hit) },
    { feature: "Pitcher Recent Form", value: Number(row.pitcher_recent_delta || 0), detail: fmtSignedDelta(row.avg_pitcher_recent_when_hit, row.avg_pitcher_recent_when_no_hit) },
    { feature: "Matchup Score", value: Number(row.matchup_delta || 0), detail: fmtSignedDelta(row.avg_matchup_when_hit, row.avg_matchup_when_no_hit) }
  ];
}

function formatFeatureName(name) {
  return String(name || "")
    .replaceAll("adjusted_", "")
    .replaceAll("_score", "")
    .replaceAll("_", " ")
...
function v2FeatureRows() {
  return (performanceV2DiagnosticRows || []).map((row) => ({
    feature: formatFeatureName(row.feature_name),
    value: Math.max(0, (Number(row.top_decile_lift || 1) - 1) * 100),
    detail: `${fmtDecimal((Number(row.top_decile_lift || 1) - 1) * 100, 1)}% lift`
  })).sort((a, b) => Math.abs(b.value) - Math.abs(a.value));
}

function computeV3FeatureRows() {
  if (!performanceV3FeatureRows || !performanceV3FeatureRows.length) return [];

  const featureMap = [
    { key: "matchup_score", label: "Matchup Score" },
    { key: "recent_form_score", label: "Recent Form" },
    { key: "batter_split_score", label: "Batter Split" },
    { key: "pitcher_vulnerability_score", label: "SP Vulnerability" },
    { key: "pitcher_recent_form_score", label: "Pitcher Recent Form" },
    { key: "batter_recent_hit_rate", label: "Recent Hit Rate" },
    { key: "pitcher_last5_whip", label: "SP Last 5 WHIP" }
  ];

  return featureMap.map((feature) => {
    let hitSum = 0, hitCount = 0, noHitSum = 0, noHitCount = 0;
    (performanceV3FeatureRows || []).forEach((row) => {
      const value = Number(row[feature.key]);
      if (!Number.isFinite(value)) return;
      if (Number(row.target_hit_1plus || 0) === 1) {
        hitSum += value;
        hitCount += 1;
      } else {
        noHitSum += value;
    ...
    const hitAvg = hitCount ? hitSum / hitCount : null;
    const noHitAvg = noHitCount ? noHitSum / noHitCount : null;
    const delta = hitAvg !== null && noHitAvg !== null ? hitAvg - noHitAvg : 0;
    return {
      feature: feature.label,
      value: Math.abs(delta),
      rawDelta: delta,
      detail: `${delta > 0 ? "+" : ""}${delta.toFixed(feature.key.includes("hit_rate") ? 3 : 1)}`
    };
  }).filter((row) => Number.isFinite(row.value)).sort((a, b) => b.value - a.value);
}

function featureRowsForModel(modelKey) {
  if (modelKey === "v1_classic") return v1FeatureRows();
  if (modelKey === "v2_proxy") return v2FeatureRows();
  if (modelKey === "v3_ml") return computeV3FeatureRows();
  return [];
}

function renderFeatureBar(value, maxValue, modelKey) {
  const pct = maxValue > 0 ? Math.max(4, Math.min(100, (Math.abs(value) / maxValue) * 100)) : 0;
  return `
    <div class="feature-bar ${performanceModelDef(modelKey).className}">
      <span style="width:${pct}%"></span>
    </div>
  `;
}

function renderFeatureImportanceCard() {
  const visible = performanceVisibleModels();
  const rows = visible.flatMap((modelKey) => featureRowsForModel(modelKey).slice(0, performanceModelFilter === "all" ? 5 : 8).map((row) => ({ ...row, model_key: modelKey })));
  ...
  return `
    <section class="board-card performance-card feature-importance-card">
      <div class="board-header">
        ...
        <div class="board-meta">
          <span>${performanceModelFilter === "v3_ml" ? `${fmtNum(performanceV3FeatureRows.length)} V3 rows` : `Last ${performanceWindow} model window`}</span>
          <span>Higher bar = stronger signal</span>
        </div>
      </div>
      <div class="table-wrap">
        <table class="performance-table feature-table">
          <thead>
            <tr>
              ${performanceModelFilter === "all" ? "<th>Model</th>" : ""}
              <th>Feature</th>
          ...
          <tbody>
            ${rows.length ? rows.map((row) => `
              <tr>
                ...
                <td>${renderFeatureBar(row.value, maxValue, row.model_key)}</td>
                <td class="num">${escapeHtml(row.detail || fmtDecimal(row.value, 1))}</td>
              </tr>
            `).join("") : `<tr><td colspan="${performanceModelFilter === "all" ? 4 : 3}" class="empty-state">No feature importance data available.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function renderPerformancePage() {
  ...
  content.innerHTML = `
    ${renderPerformanceGlobalSelector()}
    ${renderModelScorecardCard()}
    <div class="performance-two-col">
      ${renderRollingPerformanceCard()}
      ${renderScoreBucketsCard()}
    </div>
  ...
  document.querySelectorAll("#performanceModelButtons .segment").forEach((button) => {
    button.addEventListener("click", () => {
      const nextModel = button.dataset.performanceModel;
      if (!nextModel || nextModel === performanceModelFilter) return;
      performanceModelFilter = nextModel;
      renderPerformancePage();
    });
  });

  document.querySelectorAll("#performanceScorecardRangeButtons .segment").forEach((button) => {
    button.addEventListener("click", () => {
      const nextRange = button.dataset.scorecardRange;
      if (!nextRange || nextRange === performanceScorecardRange) return;
      performanceScorecardRange = nextRange;
      renderPerformancePage();
    });
  });
}
