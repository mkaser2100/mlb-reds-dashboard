...
function marketEdgeSummaryText(rows) {
  ...
  const qualified = rows.filter((row) => Number(row.edge_pct || 0) >= 5 && Number(row.predicted_probability || 0) >= 0.6 && !row.odds_stale);
  const top = rows[0];
  if (!qualified.length) {
  ...
  return `${qualified.length} qualified opportunities are live. ${qualified[0].batter_name} has the strongest edge at ${fmtPercentValue(qualified[0].edge_pct)}.`;
}
  ...
  return `
    <section class="market-hero-grid">
      ...
      <article class="insight-card compact">
        ...
        <div class="metric-sub">Model ≥ 60% · edge ≥ 5%</div>
      </article>
...
function renderMarketEdgeRows(rows) {
  if (!rows.length) {
    return `<tr><td colspan="9" class="empty-state">No market edges loaded yet. Run the hit prop odds workflow after market prices are available.</td></tr>`;
  }

  return rows.map((row, index) => `
    <tr class="clickable-row market-edge-row" data-market-player-id="${row.player_id}">
      <td class="rank"><span class="rank-badge ${index < 5 ? "rank-featured" : ""}">${row.edge_rank || index + 1}</span></td>
      <td>
        <div class="player-cell">
      ...
      <td class="num"><div class="odds-cell"><strong>${formatAmericanOdds(row.american_odds)}</strong><small>${row.book_name || "—"}</small></div></td>
...
function renderMarketEdgePage(error = null) {
  ...
  const qualified = rows.filter((row) => Number(row.edge_pct || 0) >= 5 && Number(row.predicted_probability || 0) >= 0.6 && !row.odds_stale);
  const tableRows = qualified.length ? qualified : rows.slice(0, 25);
  const showingLabel = qualified.length ? `${qualified.length} qualified opportunities` : `${Math.min(rows.length, 25)} closest market gaps`;
