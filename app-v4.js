...
const V3_DRAWER_EXPLANATION_SELECT = "prediction_id,model_run_id,target_name,prediction_run_date,game_pk,player_id,rank_overall,headline,explanation_text,positive_tags,negative_tags,generation_method,generation_status";
...
const MODEL_PERFORMANCE_FIX_VERSION = "model-performance-redesign-v5-live-feature-importance";
...
async function loadModelPerformanceData() {
  ...
  // Feature importance changes whenever V3 is retrained. Read this section
  // directly from the live view instead of trusting the performance-page cache,
  // which can remain populated with the seven legacy component signals.
  try {
    const liveFeatureRows = await mpReadView("v_mlb_app_feature_signals", {
      order: [
        { column: "model_key", ascending: true },
        { column: "feature_order", ascending: true }
      ]
    });

    if (liveFeatureRows.length) {
      modelPerformanceData.features = {
        rows: liveFeatureRows,
        sourceView: "v_mlb_app_feature_signals · live",
        refreshedAt: new Date().toISOString(),
        error: null
      };
    }
  } catch (featureErr) {
    console.error("Error loading live feature importance; using cached feature rows:", featureErr);
    modelPerformanceData.features.error = featureErr?.message || String(featureErr);
  }
