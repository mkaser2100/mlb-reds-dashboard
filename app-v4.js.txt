...
const V2_ENHANCEMENT_SELECT = "player_id,full_name,team_id,team_name,game_pk,model_v2_score,calibrated_hit_probability,probability_lift_vs_base,model_confidence,expected_plate_appearances,recent_lineup_spot,adjusted_batter_split_avg,adjusted_pitcher_baa_split,adjusted_batter_split_score,adjusted_pitcher_vulnerability_score";
let searchTerm = "";
...
function estimatedHitProbability(row) {
  if (!row) return null;

  const calibrated = Number(row.calibrated_hit_probability);
  const v2Score = Number(row.model_v2_score);

  // Supabase calibrated_hit_probability is currently bucket/decile-based.
  // That is useful for model calibration, but it causes top-decile MLB rows
  // to all display the same percentage. For row-level display, keep the
  // calibrated bucket as the baseline and use V2 Pick Score to separate
  // players within that bucket.
  if (Number.isFinite(calibrated) && Number.isFinite(v2Score)) {
    return clampNumber(calibrated + ((v2Score - 75) * 0.003), 0.35, 0.85);
  }

  if (Number.isFinite(calibrated)) {
    return clampNumber(calibrated, 0.35, 0.85);
  }

  if (Number.isFinite(v2Score)) {
    // Fallback only: approximate a probability from V2 score when calibration
    // is missing. This keeps the UI populated without pretending the score is
    // a direct probability.
    return clampNumber(0.38 + (v2Score * 0.0042), 0.35, 0.85);
  }

  return null;
}

function renderEstimatedHitProbability(row) {
  const p = estimatedHitProbability(row);
  return p != null ? `<span class="probability-badge">${fmtProbability(p)}</span>` : "—";
}
  ...
  const html = rows.map((row, index) => {
    ...
    return `
      <tr class="clickable-row" data-player-id="${row.player_id}">
        ...
        <td class="num">${renderEstimatedHitProbability(matchup)}</td>
...
function renderScoreBreakdown(matchup) {
  ...
  const v2Summary = matchup.model_v2_score != null || matchup.calibrated_hit_probability != null
    ? `
      <div class="v2-summary">
        ...
        <div>
          ...
          <strong>${fmtProbability(estimatedHitProbability(matchup))}</strong>
        </div>
        <div>
...
function openDrawer(playerId) {
  ...
  setText("drawerHitProbability", estimatedHitProbability(matchup) != null ? fmtProbability(estimatedHitProbability(matchup)) : "—");
  setText(
    "drawerPitcher",
...
function openMlbDrawer(playerId) {
  ...
  setText("drawerHitProbability", estimatedHitProbability(row) != null ? fmtProbability(estimatedHitProbability(row)) : "—");
  setText(
    "drawerPitcher",
            ...
            ${bestPitcherHitters.length ? bestPitcherHitters.map((row, index) => {
              ...
              return `
                <tr class="clickable-row mlb-clickable-row" data-mlb-player-id="${row.player_id}">
                  ...
                  <td class="num">${renderEstimatedHitProbability(row)}</td>
