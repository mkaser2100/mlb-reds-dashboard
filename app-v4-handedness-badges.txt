...
function handednessBadge(player) {
  ...
  if (hand.includes("SWITCH") || hand === "S") return "SH";
  if (hand.includes("LEFT") || hand === "L") return "LHB";
  return "RHB";
}
  ...
  try {
    const [
      ...
      client
        ...
        .select("*")
        .eq("selected_window", selectedWindow)
        .maybeSingle(),
    ...
    content.innerHTML = `
      ...
      <section class="hero-grid performance-grid">
        <article class="insight-card compact">
          ...
          <div class="metric-sub">${fmtNum(topPickPerformance?.top_1_scored)} scored</div>
        </article>
      ...
      <tr class="clickable-row" data-player-id="${row.player_id}">
        ...
        <td>
          <div class="player-cell">
            <div class="avatar">${handednessBadge(row)}</div>
            <div>
