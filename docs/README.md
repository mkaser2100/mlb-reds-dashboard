...
batter_sweet_spot_rate_30d
batter_fly_ball_rate_30d
batter_xba_30d
batter_xwoba_contact_30d
```

### Pitcher damage allowed
...
pitcher_xwoba_contact_allowed_30d
```
...
Load model artifact
        ↓
Load current power-feature rows
        ↓
Validate player/game uniqueness
        ↓
Recreate numeric + categorical preprocessing
        ↓
Logistic probability
        ↓
Platt calibration
        ↓
Build target-aware prediction rows
        ↓
Write to Supabase
        ↓
Rank predictions
        ↓
Run independent DB validator
        ↓
Refresh pipeline status
        ↓
Fail workflow if target is not COMPLETE
```
...
The current GitHub power scorer writes HR and 2+ TB.
...
This prevents duplicate player/game predictions for the same target.
...
A critical evaluation rule is:

> **DNP / void / unevaluated rows are not misses.**

Performance percentages use only rows with a valid evaluation result.

## 13.16 Frontend Serving Contract

The browser does **not** query `mlb_ml_batter_predictions` directly.
...
The public browser path is intentionally restricted.
...
11:45 UTC  Power feature refresh
    ↓
12:15 UTC  GitHub HR/TB shadow scoring
    ↓
12:30 UTC  Primary frontend serving-cache refresh
    ↓
