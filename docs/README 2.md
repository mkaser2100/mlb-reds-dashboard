# MLB Hit Lab — V3 Production Model

> **Current product, technical, and operational reference for the MLB Hit Lab V3 prediction platform**

MLB Hit Lab is now centered on the **V3 machine-learning model** for daily hit predictions, player ranking, model explanations, and Market Edge analysis.

V3 is the sole recommendation model presented in the MLB Hit Board and Team Hit Board user experiences. The MLB Hit Board provides league-wide rankings, while the Team Hit Board allows users to select any scheduled MLB game and review hitters from either team. The application previously supported three model generations—V1, V2, and V3—but the production UI has been consolidated around V3 to reduce complexity and focus engineering effort on the strongest and most scalable model architecture.
...
- Selected V2-derived matchup fields may still enrich V3 player context.
- Historical prediction tables, evaluation data, and workflows are retained for auditability, research, and rollback.
- Neither V1 nor V2 should be treated as an active standalone recommendation model in the production UI.

The current engineering strategy is to improve V3 through stronger features, better calibration, model monitoring, explainability, and dependable daily operations.
...
2. Team Hit Board
3. Why V3 Is the Production Focus
...
13. GitHub Actions and Daily Workflow
14. V3-Focused Roadmap
15. Current Deep Technical Reference
...
| Model | Current role | Standalone recommendation model in UI? |
|---|---|---:|
| V1 Classic | Historical baseline and performance benchmark | No |
| V2 Recommended | Historical benchmark and supporting matchup context | No |
| V3 ML | Production prediction, ranking, explanation, and Market Edge model | Yes |

## Active V3 Product Experiences

V3 currently powers:

- MLB Hit Board league-wide rankings
- Team Hit Board game-specific rankings for any scheduled MLB matchup
- All, Away, and Home team filtering within the selected game
- home and away starting-pitcher matchup context
- player-level hit probabilities
- confidence classifications
- player-detail explanations and model-driver analysis
- V2-enriched matchup context where applicable
- Market Edge comparisons
- current recommendation workflows

## Product Direction

The product should be described and developed as a **V3-first platform**.

V1 and V2 are retained because they provide valuable benchmarks and historical context, not because the application is still operating three competing recommendation products.

---

# 2. Team Hit Board

The Team Hit Board replaced the former Reds-only Hit Board.

Users can select any available MLB matchup and view the V3-ranked hitters for that game. The page defaults to the Cincinnati Reds game when one is available, preserving the application's original Reds-focused experience while supporting all MLB matchups.
...
Legacy Reds-specific views remain temporarily available for compatibility, parity validation, and rollback. They should not be used for new Team Hit Board development.
...
5. Treat V3 as the default model in new UI, pipeline, monitoring, and documentation work.
...
Statcast Feature Engineering
      ↓
Pitch Arsenal Feature Engineering
      ↓
Model Training
      ↓
Daily Predictions
...
V3 Serving Cache
      ↓
MLB Hit Board / Team Hit Board / Market Edge
      ↓
Actuals Loading and Evaluation
```
...
Traditional statistics answer **"What happened?"**
...
Rolling Window Features
        │
        ▼
Traditional Split Statistics
        │
        ▼
Statcast Contact Quality Layer
        │
        ▼
Pitch Arsenal Matchup Layer
        │
        ▼
Feature Engineering
        │
...
# Chapter 13 – GitHub Actions & Daily Workflow

Daily ingestion, feature generation, prediction, actual loading, evaluation, and reporting workflow.

# Chapter 14 – V3-Focused Roadmap

Near-term priorities:

1. Increase the evaluated sample size and monitor performance by rank and confidence bucket.
2. Improve probability calibration.
3. Add automated model-drift and feature-health monitoring.
4. Strengthen player-level explainability.
5. Add weather and park context.
6. Evaluate rolling Statcast trends.
7. Improve model artifact durability and reproducibility.
8. Add automated UI and pipeline smoke tests.
9. Continue reducing operational failure points in the daily prediction workflow.
10. Review V1 and V2 strictly as benchmarks unless a future research need justifies reactivation.
11. Complete the retirement of legacy Reds-specific UI wrappers, database views, scripts, and workflows after dependency and parity validation.
...
Feature glossary, feature catalog, supporting reference material, and implementation notes.

---

# Current Deep Technical Reference

> This section contains the detailed architecture, feature, pipeline, evaluation, database, and operational documentation for the active V3 platform. Individual historical notes inside this section should be treated according to their version labels.
...
V3 is MLB Hit Lab's first true supervised machine-learning prediction
engine. It predicts whether a hitter records at least one hit in a game.
...
The current Python implementation trains and evaluates multiple
scikit-learn candidate models, selects the best candidate using
validation metrics, serializes the winning model as a joblib artifact,
registers the model run in Supabase, scores today's hitter-game rows,
writes V3 predictions into Supabase, and then a GitHub Actions workflow
activates the official prediction run.

The current selected model in the latest inspected Supabase registry run
is:
...
The service role key should be used only in GitHub Actions or backend
runtime contexts, never in the browser.
...
Instead of giving the model one value per signal, V3 gives it multiple
rolling windows for each numeric feature family:
...
The wide-window design lets the model learn whether short-term or
longer-term signals matter more.
...
This is better than forcing all features into one manually chosen
lookback window.
...
This is important because the V3 model assumes one row per player-game.
Duplicate rows would leak duplicate observations into training and
corrupt ranking/scoring.
...
3.  Coercing numeric feature columns with
...
The script finds the maximum prediction date in the training data, then
creates a validation start date:
...
This is the right general approach for sports/time-series prediction
because it avoids training on future rows and validating on past rows.
...
This is appropriate for logistic regression because linear models are
sensitive to feature scale.

## 9.2 Numeric preprocessing for tree models

For Random Forest and Histogram Gradient Boosting, numeric features go
through:
...
Tree models do not require standard scaling, so the tree pipeline
imputes missing values but does not scale numeric features.
...
1.  Missing categorical values are replaced with the most frequent
    value.
2.  Categorical values are one-hot encoded.
3.  New unseen categories at scoring time are ignored instead of
    throwing an error.
...
-   It may rank well in isolated Top-N slices, but in inspected registry
    runs it did not beat logistic regression on Brier score/AUC.
...
-   In inspected registry results it underperformed logistic regression
    on calibration and AUC.
...
## 11.1 Standard ML metrics
  ...
  ----------------------- ----------------------- -----------------------
  Brier score             Mean squared error of   Lower is better
                          predicted probabilities 

  Log loss                Penalizes confident     Lower is better
                          wrong probabilities     

  ROC AUC                 Ranking/separation      Higher is better
                          ability                 

  Positive rate           Target hit rate in      Diagnostic
                          validation set          
  -----------------------------------------------------------------------
...
That matters because V3 is not just a ranker; it outputs probabilities.
A model that says `70%` should ideally hit close to 70% over enough
sample size.

The Top-10 tiebreaker aligns the selection to the product use case:
surfacing the best hitters.
...
-   Random Forest had a strong Top 5 result but lost on the primary
    selection metric.
...
The current GitHub Actions workflow creates this artifact in the runner
workspace, and the model registry stores the path. If long-term artifact
retention is required, the workflow should upload the artifact to GitHub
Actions artifacts, Supabase Storage, S3, or another durable object
store.
...
This means a trained model run is registered as a candidate, then the
workflow activates prediction rows separately.
...
The script does not currently save the exact sklearn model
hyperparameters into the `hyperparameters` JSON field. It saves the
model artifact, but the registry would be stronger if it also stored:
...
After model selection and registration, the same script scores today's
rows.
...
The script looks for the maximum available recent hit rate across
windows:
...
Rules:
  ...
  ----------------------- ----------------------- -----------------------
  `recent_rate >= 0.70`   Positive                Batter has a high
                                                  recent hit rate over
                                                  the selected window.

  `recent_rate < 0.45`    Negative                Recent hit rate is
                                                  below model baseline.
  -----------------------------------------------------------------------

## 19.2 Batter split factor

The script uses first available preferred-window value, preferring
window 10:
...
Rules:
  ...
  Condition               Direction               Text
  ----------------------- ----------------------- -----------------------
  `split_avg >= 0.280`    Positive                Batter split is
                                                  favorable for this
                                                  matchup.

  `split_avg < 0.220`     Negative                Batter split is weaker
                                                  for this matchup.
  -----------------------------------------------------------------------
...
Rules:
  ...
  Condition               Direction               Text
  ----------------------- ----------------------- -----------------------
  `whip >= 1.35`          Positive                Pitcher has allowed
                                                  traffic recently.

  `whip < 1.05`           Negative                Pitcher recent WHIP is
                                                  strong, adding risk.
  -----------------------------------------------------------------------
...
2.  Backward-compatible aliases used by existing Supabase views and UI
    drawers.
...
Before training, the workflow checks the previous day's V3 actuals
status using:
...
This matters because the training feature set can include newly
evaluated historical rows. Running training before actuals are closed
can produce stale or inconsistent training data.
...
If no rows are found, the workflow raises an error and instructs that
the upstream MLB Phase 1 data refresh must be run or fixed.
...
The workflow activates the latest model run for the target date by
calling:
...
It compares prediction `(player_id, game_pk)` pairs against batting log
`(player_id, game_pk)` pairs to estimate coverage.
...
If pending rows remain and resolved percentage is below the threshold,
the workflow fails.
...
-   Cache expensive performance payloads as JSON for the static
    frontend.
...
-   Missing pitcher features are expected in some MLB data contexts but
    should be monitored.
...
-   The workflow correctly validates target-date freshness before model
    execution.
...
The current selection rule prioritizes probability quality first, then
Top-10 ranking.
...
Based on inspected registry metrics and the current selection rule,
logistic regression is winning because:
...
Random Forest and Histogram Gradient Boosting may become stronger as
more history accumulates, but the current evidence supports logistic
regression as the safest champion/candidate.
...
The registry stores an artifact path, but the workflow does not
currently guarantee durable artifact storage.

Recommended improvement:

-   Upload joblib artifact to GitHub Actions artifacts, Supabase
    Storage, S3, or another durable store.
-   Store artifact checksum in the registry.

## 29.3 Hyperparameter persistence

The registry stores pipeline-level settings, but not exact model
hyperparameters.
...
The script stores the feature list, which is good. It does not store a
schema hash.
...
Directional insights are useful, but model promotion decisions should be
conservative until more game days accumulate.

## 29.6 No explicit probability calibration layer

The selected model outputs probabilities directly. Logistic regression
is often reasonably calibrated, but Random Forest and Gradient Boosting
may require calibration if they later become winners.
...
Add a function that returns a full model config JSON and stores it in
the registry.
...
The V3 implementation is a real production-style scikit-learn ML
pipeline.
...
The current winning model is logistic regression, not because it is the
most complex model, but because it is currently the best-calibrated and
most stable candidate for the available data.

The architecture is strong. The next maturity step is governance:
durable artifact storage, explicit champion model status, full
hyperparameter persistence, feature schema hashing, and
...
Two production-quality reliability improvements were implemented in July
2026. These changes were treated as bug fixes to the existing V3 model
rather than a new model version, so the public model remains **V3**.

### 15.1 DNP (Did Not Play) Evaluation Fix

#### Background

Originally, V1 and V2 excluded players who did not appear in a game
(DNP) when calculating daily model performance, while portions of the
evaluation pipeline could still allow DNP selections to influence
summary metrics. This created inconsistencies across model reporting and
...
-   Excludes DNP players from Top 1 / Top 5 / Top 10 / Top 20 / Top 25
    scoring.
...
No prediction probabilities or model rankings are affected by this
change. Only post-game evaluation metrics are corrected.
...
Pitchers now establish a **player-specific historical prior** using
recent MLB history before current-season performance is considered.

The hierarchy is:

    Historical MLB Prior
            ↓
    League Prior (rookies / no MLB history)
            ↓
    Current Season (gradually replaces prior)
            ↓
    Recent Form (stabilized independently)

Rather than shrinking every pitcher toward the same league average,
current-season ERA, WHIP and BAA are blended toward the pitcher's own
historical baseline. As innings accumulate during the season, current
performance naturally dominates.
...
Hitters now use the same statistical philosophy.

Small MLB samples can dramatically inflate recent batting average,
rolling hit rate and handedness splits. Recent call-ups can therefore
appear much stronger than the available MLB evidence supports.
...
Conceptually:
    ...
    adjusted_probability =
        player_prior
      + reliability × (raw_probability − player_prior)

Reliability increases continuously with effective MLB plate appearances.
...
existing prediction, dashboard and evaluation pipelines remain fully
compatible.
...
The original V3 feature engineering treated all opposing pitcher
statistics equally regardless of sample size.

This created an edge case where pitchers with only one start (or only a
few innings pitched) could produce extremely volatile statistics such
as:
...
These unstable values could artificially inflate matchup scores and
cause an unrealistic concentration of hitters facing that pitcher near
the top of the rankings.

A real-world example occurred with **Hunter Greene**, who returned from
the injured list with only **3.1 innings pitched**. His limited sample
produced exaggerated vulnerability metrics that pushed a
disproportionate number of opposing hitters into the Top 10.

#### Solution

V3 now applies empirical-Bayes style shrinkage to pitcher statistics
before they are consumed by the machine learning model.

For each pitcher, reliability is calculated from available workload
(primarily innings pitched / outs recorded). Low-reliability pitchers
are blended toward league-average prior values until enough evidence
exists for their own statistics to dominate.
...
The following existing V3 features are stabilized before model scoring
while retaining the same feature names:
...
No downstream Python feature list changes are required because the
feature contract remains unchanged.

#### Validation

Production validation confirmed:

-   Hunter Greene Top-10 exposure reduced from **6 hitters to 2
    hitters**.
-   Hunter Greene Top-25 exposure reduced from **8 hitters to 3
    hitters**.
...
This stabilization greatly improves robustness against pitchers with
extremely small seasonal workloads while preserving model behavior for
established starters.

# 16. V3 Explainability Engine (Feature-Family Diagnostics)

## Purpose

To improve transparency, debugging, and user trust, V3 now includes a
production explainability layer that summarizes **why** the model ranks
a hitter highly. This first implementation is a feature-family
diagnostic engine built directly from the stored V3 prediction features.

The explainability layer does **not** alter model predictions. It
provides an interpretable summary of the strongest positive and negative
signals that were present when the prediction was generated.

## Architecture

A new diagnostic view was added:

-   `v_mlb_v3_prediction_explanation_diagnostics`

The view evaluates stored prediction features and groups them into major
signal families rather than individual raw columns.

### Positive signal families

-   Batter Split
-   Pitcher Vulnerability
-   Pitcher Recent Form
-   Plate Appearance Volume
-   Overall Matchup Stack
-   Longer Recent Window Recovery
-   V2 Consensus

### Negative signal families

-   Poor Recent Hitter Form
-   Short-Window Cold Streak

## Diagnostic Output

For each prediction the explainability engine exposes:

-   Baseline hit probability
-   Predicted hit probability
-   Probability lift above baseline
-   Positive signal families
-   Negative signal families
-   Human-readable explanations for each signal family

## Miguel Vargas Validation

The explainability engine was validated using Miguel Vargas (July 10,
2026), who ranked #1 despite poor recent form.

Positive factors:

-   Strong platoon split (.281 over 96 AB)
-   Favorable opposing pitcher profile
-   High expected plate appearances (4.55)
-   Batting third
-   Strong overall matchup score
-   Improved 15-game trend versus the short windows
-   Independent agreement from the V2 model

Negative factors:

-   Recent AVG: .135
-   Recent hit rate: 40%
-   Recent form score: 34.6
-   Extremely weak 3-game and 5-game hot scores

The investigation confirmed that recent form was acting as a genuine
negative feature. Miguel Vargas ranked highly because multiple
independent positive feature families outweighed those negatives.

## Future Enhancement

A future enhancement will compute exact model-level feature attribution
(SHAP values) during prediction generation so every prediction can
