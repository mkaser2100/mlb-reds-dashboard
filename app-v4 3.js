...
console.info("MLB Hit Lab app-v4 loaded: model-performance-scorecard-cleanup-20260809");
...
const MODEL_PERFORMANCE_FIX_VERSION = "model-performance-redesign-v7-scorecard-baseline-fallback";
...
let modelPerformanceData = {
  ...
  dailyScorecard: { rows: [], sourceView: "v_mlb_daily_model_scorecard", error: null }
};
...
async function loadModelPerformanceData() {
  ...
  // Load daily evaluated model results so a missing cached scorecard baseline
  // (currently Last 14) can be reconstructed from actual played rows. This is
  // a display fallback only; normal cached baseline values remain authoritative.
  try {
    const dailyScorecardRows = await mpReadView("v_mlb_daily_model_scorecard", {
      order: [{ column: "prediction_run_date", ascending: false }]
    });

    modelPerformanceData.dailyScorecard = {
      rows: dailyScorecardRows,
      sourceView: "v_mlb_daily_model_scorecard · baseline fallback",
      refreshedAt: new Date().toISOString(),
      error: dailyScorecardRows.length ? null : "Daily scorecard returned zero rows."
    };
  } catch (dailyErr) {
    console.error("Daily scorecard baseline fallback failed:", dailyErr);
    modelPerformanceData.dailyScorecard = {
      rows: [],
      sourceView: "v_mlb_daily_model_scorecard · unavailable",
      refreshedAt: null,
      error: dailyErr?.message || String(dailyErr)
    };
  }
...
function mpScorecardBaselineValue(row) {
  const direct = mpRateValue(row, ["overall_hit_rate_pct", "overall_hit_rate", "hit_rate_pct", "hit_rate"]);
  if (Number.isFinite(direct)) return direct;

  // Last 14 rows in the scorecard cache currently lack overall_hit_rate_pct.
  // Reconstruct the baseline from the same model's most recent evaluated days,
  // weighting by played rows so larger slates contribute proportionally.
  const model = mpModel(row);
  const windowLabel = mpNormalizeWindowLabel(null, row);
  const maxEvaluatedDays =
    windowLabel === "Last 7" ? 7 :
    windowLabel === "Last 14" ? 14 :
    windowLabel === "Last 30" ? 30 :
    null;

  const evaluated = (modelPerformanceData.dailyScorecard?.rows || [])
    .filter((daily) =>
      mpModel(daily) === model &&
      Number(daily?.played_rows || 0) > 0 &&
      Number.isFinite(Number(daily?.overall_hits))
    )
    .slice()
    .sort((a, b) =>
      String(b?.prediction_run_date || "").localeCompare(String(a?.prediction_run_date || ""))
    );

  const selected = maxEvaluatedDays === null
    ? evaluated
    : evaluated.slice(0, maxEvaluatedDays);

  const totals = selected.reduce(
    (acc, daily) => {
      acc.hits += Number(daily?.overall_hits || 0);
      acc.played += Number(daily?.played_rows || 0);
      return acc;
    },
    { hits: 0, played: 0 }
  );

  return totals.played > 0 ? (totals.hits / totals.played) * 100 : NaN;
}
