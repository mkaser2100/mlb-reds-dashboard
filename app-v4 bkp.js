...
console.info("MLB Hit Lab app-v4 loaded: model-performance-redesign-v2-db-backed");
...
const MODEL_PERFORMANCE_FIX_VERSION = "model-performance-redesign-v2-db-backed";
...
let modelPerformanceData = {
  ...
  features: { rows: [], sourceView: "v_mlb_hit_board_component_analysis", error: null }
};
...
function mpSourceLabel(source) {
  return source?.sourceView ? `${mpInt(source.rows.length)} rows · ${source.sourceView}` : "No source view returned rows";
}

function mpFilteredScorecardRows() {
  return (modelPerformanceData.scorecard.rows || []).filter((row) => {
    const modelMatch = performanceSelectedModel === "All" || mpModel(row) === performanceSelectedModel;
    const timeMatch = mpNormalizeWindowLabel(null, row) === performanceSelectedTime;
    return modelMatch && timeMatch;
  });
}
  ...
  if (content) {
    content.innerHTML = `
      <section class="performance-note">
        ...
        <span>Reading normalized app views from Supabase.</span>
      </section>
    `;
  }

  const results = await Promise.allSettled([
    mpReadView("v_mlb_app_model_scorecard", {
      order: [{ column: "range_key", ascending: true }, { column: "model_key", ascending: true }]
    }),
    mpReadView("v_mlb_app_rolling_model_performance", {
      order: [{ column: "window_days", ascending: true }, { column: "model_key", ascending: true }]
    }),
    mpReadView("v_mlb_app_model_score_buckets", {
      order: [{ column: "model_key", ascending: true }, { column: "bucket_order", ascending: true }]
    }),
    mpReadView("v_mlb_hit_board_component_analysis", {
      eq: { selected_window: 10 },
      limit: 1
    })
  ]);

  const names = ["scorecard", "rolling", "buckets", "features"];
  const views = ["v_mlb_app_model_scorecard", "v_mlb_app_rolling_model_performance", "v_mlb_app_model_score_buckets", "v_mlb_hit_board_component_analysis"];
  results.forEach((result, index) => {
    const key = names[index];
    if (result.status === "fulfilled") {
      modelPerformanceData[key] = { rows: result.value || [], sourceView: views[index], error: null };
    } else {
      modelPerformanceData[key] = { rows: [], sourceView: views[index], error: result.reason?.message || String(result.reason) };
    }
  });
...
function mpRenderSourceGrid() {
  const items = [
    ...
    ["Buckets", modelPerformanceData.buckets],
...
function mpRenderScorecardRows() {
  const rows = mpFilteredScorecardRows();
  if (!rows.length) {
  ...
  return rows
    .slice()
    .sort((a, b) => order.indexOf(mpModel(a)) - order.indexOf(mpModel(b)))
    .map((row) => `
      <tr>
        <td><strong>${mpEscape(mpModelLabel(mpModel(row)))}</strong></td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["top1_hit_rate_pct", "top_1_hit_rate", "top_pick_hit_rate"]))}</td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["top5_hit_rate_pct", "top_5_hit_rate", "top5_hit_rate"]))}</td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["top10_hit_rate_pct", "top_10_hit_rate", "top10_hit_rate"]))}</td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["top20_hit_rate_pct", "top_20_hit_rate", "top20_hit_rate"]))}</td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["top25_hit_rate_pct", "top_25_hit_rate", "top25_hit_rate"]))}</td>
        <td class="num">${mpRateDisplay(mpRateValue(row, ["overall_hit_rate_pct", "overall_hit_rate", "hit_rate_pct", "hit_rate"]))}</td>
      </tr>
    `).join("");
}
...
function mpBucketRowsForModel(model) {
  const rows = (modelPerformanceData.buckets.rows || []).filter((row) => mpModel(row) === model);
  const sorted = rows.slice().sort((a, b) => {
    const ao = Number(mpField(a, ["bucket_order", "bucket_rank", "bucket_sort"], 999));
    const bo = Number(mpField(b, ["bucket_order", "bucket_rank", "bucket_sort"], 999));
    // V3 probability buckets increase as bucket_order increases; V1/V2 decile 1 is highest.
    return model === "V3" ? bo - ao : ao - bo;
  });
  return sorted;
}

function mpRenderBucketRows() {
  if (!modelPerformanceData.buckets.rows.length) {
    return `<tr><td colspan="4" class="empty-state">No score bucket rows returned from the database yet.</td></tr>`;
  }

  const modelBuckets = new Map(["V1", "V2", "V3"].map((model) => [model, mpBucketRowsForModel(model)]));
  return [0, 1, 2].map((index) => `
    <tr>
      <td><strong>${index === 0 ? "Highest Bucket" : `Bucket ${index + 1}`}</strong></td>
      ${["V1", "V2", "V3"].map((model) => {
        const row = modelBuckets.get(model)?.[index];
        if (!row) return `<td class="performance-bucket-cell muted">—</td>`;
        return `
          <td class="performance-bucket-cell">
            <strong>${mpRateDisplay(mpRateValue(row, ["hit_rate_pct", "hit_rate", "bucket_hit_rate"]))}</strong>
            <span>${mpEscape(mpField(row, ["bucket_label", "score_bucket", "bucket"], `Bucket ${index + 1}`))}</span>
            <small>${mpInt(mpField(row, ["players", "sample_size", "observations", "prediction_count"]))} players · ${mpInt(mpField(row, ["hits", "hitters_with_hit", "actual_hits"]))} hits</small>
          </td>
        `;
      }).join("")}
    </tr>
...
function mpFeatureRows() {
  const source = modelPerformanceData.features.rows?.[0];
  if (!source) return [];
  const components = [
  ...
  return components.map(([feature, hitKey, noHitKey]) => {
    ...
    return { model: "V1", feature, signal, hit, noHit, source };
  }).filter((row) => row.signal !== null);
}

function mpRenderFeatureRows() {
  let rows = mpFeatureRows();
  if (performanceSelectedModel !== "All") rows = rows.filter((row) => row.model === performanceSelectedModel);

  if (!rows.length) {
    return `<tr><td colspan="4" class="empty-state">No component signal rows available for ${mpEscape(performanceSelectedModel)} yet.</td></tr>`;
  }
  ...
  return rows.map((row) => {
    ...
    return `
      <tr>
        ...
        <td><strong>${mpEscape(row.feature)}</strong><div class="player-sub">Hit avg ${fmtDecimal(row.hit, 1)} · No-hit avg ${fmtDecimal(row.noHit, 1)}</div></td>
        <td>
          <div class="performance-signal-track">
            <span class="performance-signal-bar ${row.signal < 0 ? "negative" : ""}" style="width:${width}%"></span>
          </div>
        </td>
...
function renderPerformancePage() {
  ...
  content.innerHTML = `
    <section class="performance-note">
      ...
      <span>Four-card comparison layout using normalized Supabase app views. Fixed to use percent columns and app-ready view names.</span>
    </section>
    ...
    <section class="board-card performance-card">
      ...
      <div class="table-wrap">
        <table class="performance-table wide-performance-table">
          ...
          <tbody>${mpRenderRollingRows()}</tbody>
        </table>
      </div>
    </section>
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        ...
        <div class="board-meta">
          <span>Always shows all models</span>
          <span>${mpEscape(mpSourceLabel(modelPerformanceData.buckets))}</span>
        </div>
      </div>
      <div class="table-wrap">
        <table class="performance-table">
          ...
          <tbody>${mpRenderBucketRows()}</tbody>
        </table>
      </div>
    </section>
    ...
    <section class="board-card performance-card">
      <div class="board-header">
        ...
        <div class="board-meta">
          ...
          <span>${mpEscape(mpSourceLabel(modelPerformanceData.features))}</span>
        </div>
      </div>
      <div class="table-wrap">
        <table class="performance-table">
          ...
          <tbody>${mpRenderFeatureRows()}</tbody>
        </table>
      </div>
    </section>
  `;
...
function runModelPerformanceSelfTest() {
  const checks = [
    ...
    { card: "Rolling Performance", rows: modelPerformanceData.rolling.rows.length, source: modelPerformanceData.rolling.sourceView, expectedMinimumRows: 9 },
    { card: "Score Buckets", rows: modelPerformanceData.buckets.rows.length, source: modelPerformanceData.buckets.sourceView, expectedMinimumRows: 9 },
