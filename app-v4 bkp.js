...
console.info("MLB Hit Lab app-v4 loaded: v3-only-ui-focus-v5-3-opener-ui");
...
async function loadHotData() {
  try {
    ...
    // The dedicated UI view is preferred, but derive the same data from already-public
    // matchup/split sources when PostgREST has not exposed the new view yet.
    if (!redsMatchupUi) {
      await loadRedsMatchupUiFallback();
    }
...
async function loadRedsMatchupUi() {
  try {
    const { data, error } = await client
      .from("v_mlb_reds_matchup_pitcher_ui")
      .select("*")
    ...
    if (error) throw error;
    redsMatchupUi = data || null;
  } catch (err) {
    console.error("Error loading Reds matchup pitcher UI data:", err);
    redsMatchupUi = null;
  }
}

async function loadRedsMatchupUiFallback() {
  const first = matchupRows[0] || null;
  const pitcherId = first?.pitcher_id;

  if (!first || !pitcherId) return;

  try {
    const { data: splitData, error: splitError } = await client
      .from("mlb_pitcher_splits")
      .select("split_type,split_value,games,games_started,innings_pitched,hits_allowed,earned_runs,walks,batting_average_against")
    ...
    const handRows = rows.filter((row) => row.split_type === "batter_hand");
    const venueRows = rows.filter((row) => row.split_type === "venue");
    const lhb = handRows.find((row) => row.split_value === "LHB") || null;
    const rhb = handRows.find((row) => row.split_value === "RHB") || null;

    const sum = (items, field) => items.reduce((total, row) => total + Number(row?.[field] || 0), 0);
    const innings = sum(venueRows, "innings_pitched");
    const earnedRuns = sum(venueRows, "earned_runs");
    const hitsAllowed = sum(venueRows, "hits_allowed");
    const walks = sum(venueRows, "walks");
    const games = Math.max(0, ...rows.map((row) => Number(row.games || 0)));
    const gamesStarted = Math.max(0, ...rows.map((row) => Number(row.games_started || 0)));
    const role = gamesStarted === 0 ? "Opener/Reliever" : "Starter";
    const fallbackEra = innings > 0 ? (earnedRuns * 9) / innings : null;
    const fallbackWhip = innings > 0 ? (hitsAllowed + walks) / innings : null;
    const redsCount = (redsMlRows || []).filter((row) => Number(row.team_id) === TEAM_ID).length;
    const opponentCount = (redsMlRows || []).filter((row) => Number(row.team_id) !== TEAM_ID).length;

    redsMatchupUi = {
      game_pk: first.game_pk || null,
      game_date: first.game_date || null,
      season: SEASON,
      opponent_team_id: first.opponent_team_id || null,
      opponent_team_name: first.opponent_team_name || redsOpponentName(),
      probable_pitcher_id: pitcherId,
      probable_pitcher_name: first.pitcher_name || null,
      probable_pitcher_throws: first.pitcher_throws || null,
      pitcher_games: games || null,
      pitcher_games_started: gamesStarted,
      pitcher_role: role,
      fallback_season_era: fallbackEra,
      fallback_season_whip: fallbackWhip,
      baa_vs_lhb: lhb?.batting_average_against ?? null,
      baa_vs_rhb: rhb?.batting_average_against ?? null,
      reds_v3_prediction_count: redsCount,
      opponent_v3_prediction_count: opponentCount,
      metrics_unavailable_reason:
        redsCount === 0
          ? `No Reds V3 predictions were generated because matchup features were unavailable for probable pitcher ${first.pitcher_name || "the listed opposing pitcher"}, who is currently classified as an ${role.toLowerCase()}.`
          : null
    };

    console.info("Reds matchup UI fallback derived from public matchup and pitcher split data.", redsMatchupUi);
  } catch (err) {
    console.error("Error deriving Reds matchup pitcher UI fallback:", err);
  }
}
...
async function loadPitcherHandSplits() {
  ...
  const pitcherId = redsMatchupUi?.probable_pitcher_id || starter?.pitcher_id;
...
function renderMatchupHero() {
  const first = matchupRows[0] || null;
  const ui = redsMatchupUi || null;

  if (!first && !ui) {
    setText("matchupOpponent", "No matchup loaded");
    setText(
      "matchupPitcher",
      "Run the matchup loader to populate today's or next game's probable pitcher."
    );

    setText("pitcherRoleLabel", "Probable Pitcher");
    setText("pitcherEraLabel", "ERA");
    setText("pitcherWhipLabel", "WHIP");
    ...
    const notice = $("matchupDataNotice");
    if (notice) notice.hidden = true;
    return;
  }

  const gameDateRaw = ui?.game_date || first?.game_date;
  const timing = matchupTimingLabel(gameDateRaw);
  const gameDate = formatGameDate(gameDateRaw);
  const opponent = ui?.opponent_team_name || first?.opponent_team_name || redsOpponentName();
  const pitcherName = ui?.probable_pitcher_name || pitcherDisplayName(first);
  const pitcherThrows = ui?.probable_pitcher_throws || pitcherDisplayThrows(first);
  const pitcherRole = ui?.pitcher_role || "Probable Pitcher";
  const isFallback = Boolean(ui && (ui.fallback_season_era != null || ui.fallback_season_whip != null));
  const era = ui?.fallback_season_era ?? first?.pitcher_last5_era;
  const whip = ui?.fallback_season_whip ?? first?.pitcher_last5_whip;
  const baaLhb = ui?.baa_vs_lhb ?? pitcherHandSplits.lhb?.batting_average_against;
  const baaRhb = ui?.baa_vs_rhb ?? pitcherHandSplits.rhb?.batting_average_against;

  setText("matchupOpponent", `${timing}: Reds vs ${opponent || "Opponent"}`);
  setText(
    "matchupPitcher",
    `${gameDate} · ${pitcherName || "Probable pitcher TBD"}${pitcherThrows ? ` · ${pitcherThrows}HP` : ""} · ${pitcherRole}`
  );

  setText("pitcherRoleLabel", pitcherRole === "Opener/Reliever" ? "Probable Pitcher" : "Opposing Starter");
  setText("pitcherEraLabel", isFallback ? "Season ERA" : "Last 5 ERA");
  setText("pitcherWhipLabel", isFallback ? "Season WHIP" : "Last 5 WHIP");
  setText("pitcherNameCard", pitcherName || "—");
  setText("pitcherThrows", pitcherThrows || "—");
  setText("pitcherEra", fmtDecimal(era, 2));
  setText("pitcherWhip", fmtDecimal(whip, 2));
  setText("pitcherVsLhb", fmtAvg(baaLhb));
  setText("pitcherVsRhb", fmtAvg(baaRhb));

  const notice = $("matchupDataNotice");
  if (notice) {
    const reason = ui?.metrics_unavailable_reason;
    notice.hidden = !reason;
    notice.innerHTML = reason
      ? `<strong>Model availability:</strong> ${escapeHtml(reason)}`
      : "";
  }
}
...
function renderHero() {
  ...
  const teamName = top.team_name || "today's matchup";

  setText("heroPlayer", `⭐ ${playerName}`);
  setText("heroScore", `Hit Probability ${probability.toFixed(1)}% · ${confidence} confidence`);
  setText(
    "heroNarrative",
    `${playerName} leads the Reds game V3 board for ${teamName}, driven by the ML hit probability model against ${opponentPitcher}.`
  );
}
...
function currentRedsKpiRows() {
  ...
  const hotByPlayer = new Map(
    (redsGameHotRows.length ? redsGameHotRows : hotRows).map((row) => [String(row.player_id), row])
  );
...
async function loadRedsGameHotStats() {
  try {
    const teamIds = Array.from(new Set(
      redsMlRows.map((row) => row.team_id).filter(Boolean)
    ));
...
async function loadRedsGameBoardData() {
  try {
    const [mlResult] = await Promise.all([
      client
        .from("v_mlb_v3_reds_game_hit_board_cache")
        .select("*")
  ...
  return `
    <section class="control-deck performance-window-deck reds-v3-controls">
      <div class="control-group">
        ...
        <div class="model-focus-pill">⭐ V3 ML Prediction</div>
      </div>
      ...
      <div class="control-group grow">
        ...
        <div class="sort-pill">Reds vs ${escapeHtml(opponent)} · V3 hit probability</div>
      </div>

      <div class="control-group">
        <div class="control-label">Sort</div>
        <div class="sort-pill">Hit Probability ↓</div>
      </div>
    </section>
  `;
}
...
function renderRedsMlRows(rows) {
  if (!rows.length) {
    const uiReason = redsMatchupUi?.metrics_unavailable_reason;
    let message = "No V3 hitters match the current filter.";

    if (redsTeamFilter === "reds" && Number(redsMatchupUi?.reds_v3_prediction_count || 0) === 0) {
      message = uiReason || `No Reds V3 predictions were generated for ${redsMatchupUi?.probable_pitcher_name || "the probable opposing pitcher"}.`;
    } else if (redsTeamFilter === "opponent" && Number(redsMatchupUi?.opponent_v3_prediction_count || 0) === 0) {
      message = "No opponent V3 predictions were generated for this matchup.";
    }

    return `<tr><td colspan="9" class="empty-state empty-state-explained"><strong>No hitters available</strong><span>${escapeHtml(message)}</span></td></tr>`;
  }
...
function renderTable() {
  ...
  setText("redsBoardTitle", `Reds vs ${opponent} Hit Probabilities`);
  setText("redsBoardEyebrow", "Game-Specific V3 Board");
  setText("statusPill", `${fmtNum(redsMlRows.length)} hitters scored`);
  setText("lastRefresh", "V3 ML Prediction");
...
function openV3MlbDrawer(row) {
  ...
  const positiveDrivers = row.drawer_top_positive_drivers || [];
  const riskFactors = row.drawer_top_risk_factors || [];
  const hasExactContributions = positiveDrivers.length || riskFactors.length;
  const driverSummary = v3DriverSummary(row, positiveDrivers, riskFactors);
  ...
  body.innerHTML = `
    ...
    <section class="drawer-section v3-section v3-driver-section">
      <div class="drawer-section-title">Why the model likes him</div>
      ${positiveDrivers.length
        ? v3DriverRows(positiveDrivers, "positive")
        : `<div class="v3-driver-contract-error">
            <strong>Driver details unavailable</strong>
            <span>This prediction did not satisfy the unified V3 drawer data contract.</span>
          </div>`}

      ${riskFactors.length ? `
        <div class="v3-driver-divider"></div>
        <div class="v3-driver-section-label">Risks to watch</div>
        ${v3DriverRows(riskFactors, "negative")}
      ` : `
        <div class="v3-driver-divider"></div>
        <div class="v3-driver-section-label">Risks to watch</div>
        <div class="v3-driver-contract-error compact">
          <span>No structured risk factors were returned for this prediction.</span>
        </div>
      `}

      ${explanationHeadline ? `<div class="v3-explanation-headline">${escapeHtml(explanationHeadline)}</div>` : ""}
      <p class="drawer-explanation v3-driver-summary">${escapeHtml(driverSummary || explanation)}</p>
    </section>
...
function renderMlbPredictionModeControls() {
  return `
    <section class="control-deck performance-window-deck v3-control-deck">
      ...
      <div class="control-group grow">
        <div class="control-label">Scope</div>
        <div class="sort-pill">All MLB hitters · ranked by V3 hit probability</div>
      </div>
...
function showView(viewName) {
  ...
  if (viewName === "performance") {
    ...
    setText("pageSubtitle", "Compare V1, V2, and V3 results across scorecard windows, stability, calibration, and feature signals.");
...
// Weighted overall hit-rate baselines for the latest 14 fully evaluable game dates.
// The rolling Last 14 rows contain bucket rates but not the overall baseline needed
// to render the “+/- pts vs baseline” subtext.
const mpLast14BaselineByModel = new Map();
...
async function mpLoadLast14Baselines() {
  try {
    const { data, error } = await client
      .from("v_mlb_daily_model_scorecard")
      .select(
        "prediction_run_date,model_key,model_name,played_rows,overall_hits," +
        "pending_or_unloaded_rows"
      )
      .order("prediction_run_date", { ascending: false })
      .limit(250);
    ...
    // Use the latest 14 dates that are fully evaluable and have played rows.
    // The same date set is shared across models so the comparison is consistent.
    const evaluableDates = [...new Set(
      rows
        .filter((row) =>
          Number(row.pending_or_unloaded_rows || 0) === 0 &&
          Number(row.played_rows || 0) > 0
        )
        .map((row) => String(row.prediction_run_date || ""))
        .filter(Boolean)
    )].slice(0, 14);

    const selectedDates = new Set(evaluableDates);
    const totalsByModel = new Map();

    rows
      .filter((row) => selectedDates.has(String(row.prediction_run_date || "")))
      .forEach((row) => {
        const model = mpModel(row);
        if (!["V1", "V2", "V3"].includes(model)) return;

        if (!totalsByModel.has(model)) {
          totalsByModel.set(model, { hits: 0, played: 0 });
        }

        const totals = totalsByModel.get(model);
        totals.hits += Number(row.overall_hits || 0);
        totals.played += Number(row.played_rows || 0);
      });

    mpLast14BaselineByModel.clear();

    totalsByModel.forEach((totals, model) => {
      if (totals.played > 0) {
        mpLast14BaselineByModel.set(
          model,
          (totals.hits / totals.played) * 100
        );
      }
    });

    console.info("Loaded weighted Last 14 scorecard baselines", {
      evaluableDates,
      baselines: Object.fromEntries(mpLast14BaselineByModel)
    });
  } catch (err) {
    // Keep the rest of Model Performance usable if this supplemental query fails.
    // The Last 14 lift text will remain hidden rather than showing an incorrect value.
    mpLast14BaselineByModel.clear();
    console.error("Unable to load Last 14 scorecard baselines:", err);
  }
}
...
function mpScorecardBaselineValue(row) {
  const storedBaseline = mpRateValue(row, [
  ...
  if (Number.isFinite(storedBaseline)) return storedBaseline;

  if (performanceSelectedTime === "Last 14") {
    const calculatedBaseline = mpLast14BaselineByModel.get(mpModel(row));
    if (Number.isFinite(calculatedBaseline)) return calculatedBaseline;
  }
