...
SCRIPT_VERSION = "v5_hydrate_player_handedness"
...
def ensure_players_exist(all_logs: List[Dict[str, Any]]):
    ...
    for row in existing_response.data:
        ...
        needs_refresh = (
            row.get("bats") is None
            or row.get("throws") is None
            or full_name.startswith("Historical Reds Player")
            or full_name.startswith("Unknown Player")
            or full_name.startswith("Player ")
        )

        if needs_refresh:
            players_needing_refresh.append(row["player_id"])
    ...
    refresh_ids = sorted(list(set(missing_ids + players_needing_refresh)))

    print(f"Missing historical players found: {len(missing_ids)}")
    print(f"Existing players needing handedness/name refresh: {len(players_needing_refresh)}")
    print(f"Total players to hydrate from MLB People API: {len(refresh_ids)}")

    if refresh_ids:
        print(f"Player IDs to hydrate: {refresh_ids}")

    missing_player_rows = []

    for player_id in refresh_ids:
        try:
