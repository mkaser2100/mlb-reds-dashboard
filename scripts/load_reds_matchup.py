...
SCRIPT_VERSION = "v1_2_reds_matchup_loader_next_game_safe"
...
def local_game_date(game_datetime: Optional[str]) -> str:
    """
    Convert MLB's UTC gameDate into the local Eastern date that Reds fans expect.

    Example:
    2026-06-06T00:15:00Z is Friday night, 2026-06-05, in Cincinnati/New York time.
    """
    ...
    try:
        ...
        print(f"Unable to convert gameDate to local date: {game_datetime}. Error: {exc}")
        return str(game_datetime)[:10]
...
def get_today_reds_game() -> Optional[Dict[str, Any]]:
    ...
    if not dates:
        print("No Reds game found today in MLB schedule endpoint.")
        return None

    games = dates[0].get("games", [])

    if not games:
        print("Schedule response had a date entry but no Reds games.")
        return None
...
def get_next_reds_game(days_ahead: int = 10) -> Optional[Dict[str, Any]]:
    start_date = datetime.now(timezone.utc).date()
    end_date = start_date + timedelta(days=days_ahead)

    print(f"Looking for next Reds game from {start_date.isoformat()} through {end_date.isoformat()}")
    ...
    if not games:
        print("No upcoming Reds game found in lookahead window.")
        return None

    games.sort(key=lambda g: str(g.get("gameDate") or ""))
    game = games[0]

    print(
        f"Found next Reds game game_pk={game.get('gamePk')} "
        f"date={str(game.get('gameDate'))[:10]} "
        f"status={game.get('status', {}).get('detailedState')}"
    )
...
def daily_matchup_row(game: Dict[str, Any], side: Dict[str, Any], pitcher: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "game_pk": game["gamePk"],
        "game_date": local_game_date(game.get("gameDate")),
        "season": SEASON,
...
def main():
    ...
    if not game:
        print("No Reds game today.")
        print("Checking for the next Reds game so the app can show a future matchup instead of stale data.")
        game = get_next_reds_game(days_ahead=10)
        game_context = "next"

    if not game:
        print("No Reds game today and no upcoming Reds game found in the lookahead window.")
        print("Matchup loader exiting successfully.")
        return

    print(f"Using {game_context} Reds game for matchup card.")
    ...
    if not probable_pitcher_stub:
        print(f"No opposing probable pitcher listed yet for {game_context} Reds game.")
