...
def register_model(client: Any, cfg: Config, model_name: str, metrics: dict[str, Any], train_df: pd.DataFrame, valid_df: pd.DataFrame, artifact_uri: str) -> int | None:
    payload = {
        ...
        "notes": "V3 supervised ML candidate for hit_1plus using wide window features: one row per player-game with w3/w5/w6/w10/w15 feature columns. Metrics include row-weighted Top N and daily-average Top N hit rates.",
    }
    if cfg.dry_run:
