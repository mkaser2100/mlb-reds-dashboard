...
Maintenance:
  - Calls public.cleanup_mlb_v3_explanation_history after a successful score
    to retain only the latest three calendar dates of drawer explanations.
...
# Lineup spot is now an explicit model feature. Historical MLB batting-order
# encodings are normalized to 1-9 before training.
LINEUP_NUMERIC_FEATURES = ["batting_order"]

NUMERIC_FEATURES = (
    WINDOW_NUMERIC_FEATURES
    + MATCHUP_NUMERIC_FEATURES
    + LINEUP_NUMERIC_FEATURES
)
...
def normalize_and_validate_lineup(
    ...
    *,
    ...
    if "batting_order" not in frame.columns:
        raise ValueError(f"{context} data is missing required batting_order.")

    clean = frame.copy()
    clean["batting_order"] = normalize_lineup_slot(clean["batting_order"])

    total = int(len(clean))
    valid = int(clean["batting_order"].notna().sum())
    missing = total - valid
    ...
    if require_full_coverage and missing:
        sample_cols = [
            col
            for col in ["prediction_run_date", "game_pk", "player_id", "batter_name"]
            if col in clean.columns
        ]
        sample = (
            clean.loc[clean["batting_order"].isna(), sample_cols]
            .head(20)
            .to_dict("records")
        )
        raise ValueError(
...
def ensure_matchup_features_ready(
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            counts_ready = (
                missing_rows is not None
                and int(missing_rows) == 0
                and expected_rows is not None
                and existing_rows is not None
                and int(existing_rows) >= int(expected_rows)
            )
        except (TypeError, ValueError):
            counts_ready = False

        terminal_ready = status == "ready" or (
            status in {"refreshed", "complete"} and counts_ready
        )

        if terminal_ready:
            if missing_rows not in (None, 0):
                raise RuntimeError(
                    "Matchup readiness contract returned a terminal status "
                    f"with missing rows: response={payload}"
                )
            print(
                "Matchup features ready: "
                f"status={status}, game_date={game_date}, "
                f"existing={existing_rows}, expected={expected_rows}, "
                f"missing={missing_rows or 0}"
            )
            return payload

        if status in retryable_statuses.union({"refreshed"}) and attempt < attempts:
            print(
...
def fetch_matchup_features(client: Any) -> pd.DataFrame:
    columns = (
        MATCHUP_JOIN_KEYS
        + MATCHUP_NUMERIC_FEATURES
        + ["effective_batter_side", "feature_version"]
    )
    ...
    if duplicate_mask.any():
        sample = (
            matchup.loc[duplicate_mask, MATCHUP_JOIN_KEYS]
            .head(20)
            .to_dict("records")
        )
        raise ValueError(
...
def attach_matchup_features(
    ...
    if unmatched_rows:
        ...
        print(
            f"WARNING: {message} These historical rows remain in training with "
    ...
    # Normalize historical 100/200/... encodings and enforce current-day coverage.
    merged = normalize_and_validate_lineup(
        merged,
        context,
    ...
    if duplicate_mask.any():
        sample = (
            df.loc[
                ...
                + [
                    c
                    for c in ["batter_name", "team_name"]
                    if c in df.columns
                ],
            ]
            .head(20)
...
def first_available_window_value(
    row: pd.Series,
    base: str,
    preferred_window: int = 10,
) -> tuple[Any, int | None]:
...
def max_available_window_value(
    row: pd.Series,
    base: str,
) -> tuple[Any, int | None]:
    best_value: Any = None
...
def coerce_training_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    clean = df.copy()
    clean["prediction_run_date"] = pd.to_datetime(
        clean["prediction_run_date"]
    ).dt.date
    ...
    return clean.dropna(
        subset=["prediction_run_date", "target_hit_1plus"]
    )
...
def split_time_based(
    ...
    validation_start = pd.to_datetime(max_date) - pd.Timedelta(
        days=validation_days - 1
    )
    validation_start_date = validation_start.date()

    train_df = df[
        df["prediction_run_date"] < validation_start_date
    ].copy()
    valid_df = df[
        df["prediction_run_date"] >= validation_start_date
    ].copy()
    ...
    if train_df.empty or valid_df.empty:
        raise ValueError(
            "Time split produced empty train or validation set. "
            "Increase data history or reduce V3_VALIDATION_DAYS."
        )
...
def evaluate_predictions(
    ...
    eval_df = valid_df[
        ["prediction_run_date", "player_id", "batter_name", "team_name"]
    ].copy()
    eval_df["actual"] = y_true
    eval_df["prob"] = prob
    eval_df["rank"] = eval_df.groupby("prediction_run_date")["prob"].rank(
        method="first",
        ascending=False,
    )
    ...
    for k in [1, 5, 10, 20, 25]:
        ...
        out[f"top_{k}_hit_rate"] = (
            None if subset.empty else float(subset["actual"].mean())
        )

        daily_rates = (
            subset.groupby("prediction_run_date")["actual"].mean()
            if not subset.empty
            else pd.Series(dtype=float)
        )
        out[f"daily_top_{k}_days"] = int(daily_rates.count())
        out[f"daily_top_{k}_hit_rate"] = (
            None if daily_rates.empty else float(daily_rates.mean())
        )
        out[f"daily_top_{k}_min_hit_rate"] = (
            None if daily_rates.empty else float(daily_rates.min())
        )
        out[f"daily_top_{k}_max_hit_rate"] = (
            None if daily_rates.empty else float(daily_rates.max())
        )
...
def contribution_explanation(
    ...
    if raw_value is not None and math.isfinite(raw_value):
        if any(
            ...
            for token in ("avg", "xba", "xwoba", "rate", "pct", "reliability")
        ):
            value_text = f" Current value: {raw_value:.3f}."
        else:
            value_text = f" Current value: {raw_value:.2f}."
    return (
        f"{display_name} {direction_text} this player's modeled hit probability."
        f"{value_text}"
    )


def prediction_identity_key(
    ...
    game_pk = (
        None
        if game_value is None or pd.isna(game_value)
        else int(game_value)
    )
    return prediction_date, player_id, game_pk


def _dense_transformed_features(
    model: Pipeline,
    frame: pd.DataFrame,
    ...
    if transformed.shape[1] != len(transformed_names):
        raise RuntimeError(
            "Transformed feature values and feature names are misaligned while "
            "calculating player-level explanations."
        )
    return transformed, transformed_names, source_features
...
def _positive_class_shap_values(
    ...
    if isinstance(values, list):
        if not values:
            raise RuntimeError("SHAP returned an empty value list.")
        values = values[1] if len(values) > 1 else values[0]

    array = np.asarray(values, dtype=float)
    if array.ndim == 2 and array.shape == (
        row_count,
        feature_count,
    ):
        return array
    if array.ndim == 3:
        if (
            ...
            and array.shape[1] == feature_count
        ):
            return array[:, :, 1 if array.shape[2] > 1 else 0]
        if (
            array.shape[1] == row_count
            and array.shape[2] == feature_count
        ):
            return array[
                1 if array.shape[0] > 1 else 0,
                :,
                :,
            ]
    ...
    raise RuntimeError(
        ...
        f"received={array.shape}, expected rows={row_count}, "
        f"features={feature_count}."
    )


def calculate_model_agnostic_player_contributions(
    model_name: str,
    model: Pipeline,
    today: pd.DataFrame,
    ...
    transformed, _, source_features = _dense_transformed_features(
        model,
        today,
    )
    ...
    if background_df.empty:
        raise RuntimeError(
            "Cannot calculate SHAP explanations with an empty background set."
        )
    background_sample = background_df.sample(
        n=min(200, len(background_df)),
        random_state=42,
    )
    background, _, background_source_features = _dense_transformed_features(
        model,
        background_sample,
    )
    if background_source_features != source_features:
        raise RuntimeError(
            "Scoring and SHAP background transformed features are misaligned."
        )
    ...
    if model_name == "logistic_regression":
        ...
        calculation_method = "linear_shap_raw_score"
        ...
        calculation_method = "tree_shap_probability"
    else:
        try:
            ...
            explanation = explainer(transformed)
            calculation_method = "shap_auto"
        except Exception as native_exc:
            print(
                f"Native SHAP explainer failed for {model_name}: "
                f"{native_exc}. Falling back to permutation SHAP."
            )
            explainer = shap.Explainer(
            ...
            explanation = explainer(transformed)
            calculation_method = "permutation_shap_probability"
    ...
    if not np.isfinite(shap_values).all():
        raise RuntimeError(
            "SHAP produced non-finite player-level contribution values."
        )
    ...
    for row_position, (_, source_row) in enumerate(today.iterrows()):
        ...
        for column_position, feature_name in enumerate(source_features):
            contribution = float(
                shap_values[row_position, column_position]
            )
            transformed_value = float(
                transformed[row_position, column_position]
            )
            grouped_contribution[feature_name] = (
                grouped_contribution.get(feature_name, 0.0)
                + contribution
            )
            grouped_transformed[feature_name] = (
                grouped_transformed.get(feature_name, 0.0)
                + transformed_value
            )

        total_absolute = sum(
            abs(value)
            for value in grouped_contribution.values()
        )
        ordered = sorted(
            ...
            player_rows.append(
                {
                    ...
                    "contribution_abs": abs(contribution_value),
                    "contribution_pct": (
                        abs(contribution_value)
                        / total_absolute
                        * 100.0
                        if total_absolute > 0
                        else 0.0
...
def persist_player_feature_contributions(
    ...
    contribution_sets = calculate_model_agnostic_player_contributions(
    ...
    if cfg.dry_run:
        print(
            ...
            f"{sum(len(rows) for rows in contribution_sets)} "
            "model-agnostic player-feature contribution rows."
        )
        print(
            json.dumps(
                contribution_sets[0][:5]
                if contribution_sets
                else [],
                indent=2,
                default=str,
            )
    ...
    if model_run_id is None:
        raise RuntimeError(
            "Cannot persist player contributions without model_run_id."
        )

    prediction_id_by_key: dict[
        tuple[str, int, int | None],
        int,
    ] = {}
    for inserted in inserted_predictions:
        if inserted.get("prediction_id") is None:
            continue
        prediction_id_by_key[
            prediction_identity_key(inserted)
        ] = int(inserted["prediction_id"])

    if len(prediction_id_by_key) != len(today):
        missing_keys = [
            prediction_identity_key(row)
            for _, row in today.iterrows()
            if prediction_identity_key(row)
            not in prediction_id_by_key
        ]
        raise RuntimeError(
            ...
            payload.append(
                {
                    ...
                    "raw_feature_value": (
                        ...
                        if item["raw_feature_value"] is None
                            ...
                            float(item["raw_feature_value"]),
                            10,
                        )
                    ),
                    "transformed_feature_value": round(
                        float(
                            item["transformed_feature_value"]
                        ),
                        10,
                    ),
                    "contribution_value": round(
                        float(item["contribution_value"]),
                        10,
                    ),
                    "contribution_pct": round(
                        float(item["contribution_pct"]),
                        6,
                    ),
    ...
    print(
        f"Persisted {len(payload)} model-agnostic SHAP contribution rows "
...
def calculate_logistic_feature_importance(
    ...
    if (
        transformed.shape[1] != len(coefficients)
        or len(coefficients) != len(transformed_names)
    ):
        raise RuntimeError(
            "Transformed feature names and logistic-regression coefficients "
            "are misaligned."
        )

    absolute_contribution = np.mean(
        np.abs(transformed * coefficients),
        axis=0,
    )
    signed_contribution = np.mean(
    ...
    for transformed_name, absolute_value, signed_value in zip(
        ...
        grouped_absolute[feature_name] = (
            grouped_absolute.get(feature_name, 0.0)
            + float(absolute_value)
        )
        grouped_signed[feature_name] = (
            grouped_signed.get(feature_name, 0.0)
            + float(signed_value)
        )
...
def calculate_permutation_feature_importance(
    ...
    return [
        ...
        for feature_name, importance_value
        in zip(FEATURES, result.importances_mean)
    ]
...
def persist_feature_importance(
    ...
    if model_run_id is None:
        print(
            "DRY RUN skipping feature-importance database write."
        )
        return

    if valid_df.empty:
        raise RuntimeError(
            "Cannot calculate V3 feature importance from an empty validation set."
        )

    if model_name == "logistic_regression":
        importance_rows = calculate_logistic_feature_importance(
            model,
            valid_df,
        )
    else:
        importance_rows = calculate_permutation_feature_importance(
            model,
            valid_df,
        )
    ...
    if total_importance <= 0:
        raise RuntimeError(
            "Calculated V3 feature importance was zero for every feature."
        )

    payload: list[dict[str, Any]] = []
    for feature_order, row in enumerate(
        importance_rows,
        start=1,
    ):
    ...
    for row in payload[:15]:
        print(
            f"{row['feature_order']:>2}. "
            f"{row['feature_name']:<45} "
            f"{row['importance_pct']:>8.4f}%  "
            f"{row['signal_method']}"
        )
...
def build_explanation(
    ...
    recent_rate, recent_window = max_available_window_value(
        row,
        "batter_recent_hit_rate",
    )
    if recent_rate is not None:
        ...
        if value >= 0.70:
            add_factor(
                ...
                value,
                "positive",
                f"Batter has a high recent hit rate over the last "
                f"{recent_window} games.",
                recent_window,
            )
        elif value < 0.45:
            add_factor(
                ...
                value,
                "negative",
                f"Recent hit rate is below the model baseline over the last "
                f"{recent_window} games.",
                recent_window,
            )

    split_avg, split_window = first_available_window_value(
        row,
        "batter_split_avg",
    )
    if split_avg is not None:
        ...
        if value >= 0.280:
            add_factor(
                ...
                value,
                "positive",
                "Batter split is favorable for this matchup.",
                split_window,
            )
        elif value < 0.220:
            add_factor(
                ...
                value,
                "negative",
                "Batter split is weaker for this matchup.",
                split_window,
            )

    pitcher_baa, pitcher_baa_window = first_available_window_value(
        row,
        "pitcher_baa_split",
    )
    ...
    whip, whip_window = first_available_window_value(
        row,
        "pitcher_last5_whip",
    )
    if whip is not None:
        ...
        if value >= 1.35:
            add_factor(
                ...
                value,
                "positive",
                "Pitcher has allowed traffic recently.",
                whip_window,
            )
        elif value < 1.05:
            add_factor(
                ...
                value,
                "negative",
                "Pitcher recent WHIP is strong, adding risk.",
                whip_window,
            )
    ...
    if not explanation:
        explanation = (
            "Projection is driven by the wide V3 feature mix across "
            "recent-game windows."
        )
...
def build_prediction_features(
    ...
    for alias, base in alias_map.items():
        value, window = first_available_window_value(
            row,
            base,
            preferred_window=10,
        )
    ...
    features["matchup_feature_version"] = json_safe(
...
def register_model(
    ...
    payload = {
        ...
        "training_start_date": str(
            train_df["prediction_run_date"].min()
        ),
        "training_end_date": str(
            train_df["prediction_run_date"].max()
        ),
        "validation_start_date": str(
            valid_df["prediction_run_date"].min()
        ),
        "validation_end_date": str(
            valid_df["prediction_run_date"].max()
        ),
        ...
        "hyperparameters": {
            ...
            "windows": WINDOWS,
            "lineup_feature": "batting_order_normalized_1_to_9",
        },
        "metrics": metrics,
        "artifact_uri": artifact_uri,
        "notes": (
            "V3 supervised ML candidate for hit_1plus using wide rolling-window "
            "features, point-in-time contact-quality and pitch-arsenal matchup "
            "features, plus explicit normalized batting-order position. "
    ...
    response = client.table(MODEL_RUNS_TABLE).insert(
        payload
    ).execute()
    if not response.data:
        raise RuntimeError(
            "Model registration failed; no row returned."
        )
...
def score_today(
    ...
    matchup_df: pd.DataFrame,
    today: pd.DataFrame,
    explanation_background_df: pd.DataFrame,
) -> pd.DataFrame:
    ...
    validate_no_duplicate_player_games(
        today,
        "Today",
    )

    for col in NUMERIC_FEATURES:
        today[col] = pd.to_numeric(
            today[col],
            errors="coerce",
        )
    for col in CATEGORICAL_FEATURES:
    ...
    today["predicted_probability"] = model.predict_proba(
        today[FEATURES]
    )[:, 1]
    today["rank_overall"] = (
        today["predicted_probability"]
        .rank(
            method="first",
            ascending=False,
        )
    ...
    today["rank_team"] = (
        today.groupby("team_id")["predicted_probability"]
        .rank(
            method="first",
            ascending=False,
        )
    ...
    for _, row in today.iterrows():
        ...
        confidence, factors, explanation = build_explanation(
            row,
            prob,
        )
        prediction_rows.append(
            {
                "model_run_id": model_run_id,
                "target_name": TARGET_NAME,
                "prediction_run_date": str(
                    row["prediction_run_date"]
                ),
                "game_date": (
                    None
                ...
                "team_id": (
                    ...
                    else int(row["team_id"])
                ),
                "team_name": row.get("team_name"),
                "pitcher_id": (
                    None
                    if pd.isna(row.get("pitcher_id"))
                    else int(row["pitcher_id"])
                ),
                "pitcher_name": row.get("pitcher_name"),
                "pitcher_team_name": row.get(
                    "pitcher_team_name"
                ),
    ...
    if cfg.dry_run:
        ...
        for start in range(
            ...
            if (
                not response.data
                or len(response.data) != len(chunk)
            ):
                raise RuntimeError(
                    "Prediction insert did not return every inserted row. "
                    f"expected={len(chunk)}, "
                    f"returned={len(response.data or [])}"
                )
            inserted_predictions.extend(
                response.data
            )
    ...
    persist_player_feature_contributions(
        ...
        model_run_id=model_run_id,
        today=today,
        background_df=explanation_background_df,
...
def cleanup_explanation_history(
    client: Any,
    cfg: Config,
    scoring_date: str,
) -> None:
    if cfg.dry_run:
        print(
            "DRY RUN would call cleanup_mlb_v3_explanation_history "
            f"for reference_date={scoring_date}, "
            f"keep_days={EXPLANATION_RETENTION_DAYS}."
        )
        return

    response = client.rpc(
        "cleanup_mlb_v3_explanation_history",
        {
            "p_reference_date": scoring_date,
            "p_keep_days": EXPLANATION_RETENTION_DAYS,
        },
    ).execute()

    result = response.data or {}
    print(
        "V3 explanation retention cleanup completed: "
        f"reference_date={scoring_date}, "
        f"keep_days={EXPLANATION_RETENTION_DAYS}, "
...
def main() -> None:
    ...
    print(
        f"Loaded {len(matchup_df)} rows from "
        f"{MATCHUP_FEATURES_TABLE}."
    )
    ...
    print(
        f"Training coverage: rows={len(df)}, "
        f"dates={df['prediction_run_date'].nunique()}, "
        f"start={df['prediction_run_date'].min()}, "
        f"end={df['prediction_run_date'].max()}"
    )

    if len(df) < cfg.min_train_rows:
        raise RuntimeError(
            f"Only {len(df)} training rows found; "
            f"minimum is {cfg.min_train_rows}."
        )

    train_df, valid_df = split_time_based(
        df,
        cfg.validation_days,
    )
    ...
    artifact_path = (
        ARTIFACT_DIR
        / f"{cfg.model_version}_{best_name}.joblib"
    )
    joblib.dump(
        {
            "model": best_model,
