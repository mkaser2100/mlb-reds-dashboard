...
def ensure_matchup_features_ready(
    ...
    for attempt in range(1, attempts + 1):
        ...
        # The readiness RPC returns "ready" when no refresh is needed and
        # "refreshed" after it successfully builds the missing rows. Both are
        # terminal success states when the row counts prove full coverage.
...
def _dense_transformed_features(model: Pipeline, frame: pd.DataFrame) -> tuple[np.ndarray, list[str], list[str]]:
    """Transform model inputs and return dense values plus transformed/source names."""
...
def _positive_class_shap_values(values: Any, row_count: int, feature_count: int) -> np.ndarray:
    """Normalize SHAP's version/model-specific binary-class output to rows x features."""
    ...
    if array.ndim == 2 and array.shape == (row_count, feature_count):
        return array
    if array.ndim == 3:
        # Common binary-class layouts are rows x features x classes or
        # classes x rows x features. Always explain the positive class.
        if array.shape[0] == row_count and array.shape[1] == feature_count:
            return array[:, :, 1 if array.shape[2] > 1 else 0]
        if array.shape[1] == row_count and array.shape[2] == feature_count:
            return array[1 if array.shape[0] > 1 else 0, :, :]

    raise RuntimeError(
        "Unsupported SHAP output shape. "
        f"received={array.shape}, expected rows={row_count}, features={feature_count}."
    )
...
def calculate_model_agnostic_player_contributions(
    ...
    """Calculate signed local feature contributions for any supported classifier.

    SHAP is run against the fitted estimator after the pipeline preprocessing step.
    The output is normalized back to original source features so the existing table,
    views, and player drawer remain unchanged.
    """
    estimator = model.named_steps["model"]
    transformed, _, source_features = _dense_transformed_features(model, today)

    if background_df.empty:
        raise RuntimeError("Cannot calculate SHAP explanations with an empty background set.")
    ...
    if background_source_features != source_features:
        raise RuntimeError("Scoring and SHAP background transformed features are misaligned.")
    ...
    if model_name == "logistic_regression":
        ...
        # SHAP's unified interface selects a native tree explainer when supported
        # and falls back to a model-agnostic algorithm for unfamiliar estimators.
        try:
            ...
            print(
                f"Native SHAP explainer failed for {model_name}: {native_exc}. "
                "Falling back to permutation SHAP."
            )
    ...
    if not np.isfinite(shap_values).all():
        raise RuntimeError("SHAP produced non-finite player-level contribution values.")
    ...
    for row_position, (_, source_row) in enumerate(today.iterrows()):
        ...
        for column_position, feature_name in enumerate(source_features):
            contribution = float(shap_values[row_position, column_position])
        ...
        ordered = sorted(
            grouped_contribution.items(), key=lambda item: abs(item[1]), reverse=True
        )
        ...
        for feature_rank, (feature_name, contribution_value) in enumerate(ordered, start=1):
            ...
            if feature_name in source_row.index:
                candidate = pd.to_numeric(
                    pd.Series([source_row.get(feature_name)]), errors="coerce"
                ).iloc[0]
            ...
            player_rows.append(
                {
                    ...
                    "contribution_pct": (
                        abs(contribution_value) / total_absolute * 100.0
                        if total_absolute > 0
                        else 0.0
                    ),
                    ...
                    "explanation_text": contribution_explanation(
                        feature_name, display_name, raw_value, direction
                    ),
...
def persist_player_feature_contributions(
    ...
    """Persist model-agnostic SHAP contributions for the predictions just inserted."""
    ...
    if cfg.dry_run:
        print(
            ...
            f"{sum(len(rows) for rows in contribution_sets)} model-agnostic "
            "player-feature contribution rows."
        )
        print(json.dumps(contribution_sets[0][:5] if contribution_sets else [], indent=2, default=str))
        return
            ...
            payload.append(
                {
                    ...
                    "contribution_value": round(float(item["contribution_value"]), 10),
                    "contribution_abs": round(float(item["contribution_abs"]), 10),
...
def cleanup_explanation_history(client: Any, cfg: Config, scoring_date: str) -> None:
    """Keep only the current scoring date and the prior two calendar dates."""
    if cfg.dry_run:
        print(
            "DRY RUN would call cleanup_mlb_v3_explanation_history "
            f"for reference_date={scoring_date}, keep_days={EXPLANATION_RETENTION_DAYS}."
        )
        return
    ...
    print(
        ...
        f"reference_date={scoring_date}, keep_days={EXPLANATION_RETENTION_DAYS}, "
...
def main() -> None:
    ...
    scored = score_today(
        client, cfg, best_name, best_model, model_run_id, matchup_df, today, valid_df
    )
    cleanup_explanation_history(client, cfg, scoring_date)
